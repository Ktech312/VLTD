...
Ten items EK explicitly asked to defer to "the next batch of fixes" (the last one queued explicitly to run AFTER the rest). Status after this session's continuous work-through-the-list pass — 8 of 9 fixed and deployed (commits `a99c321`, `fa51153`, `77eff75`, `d1815e0`), only the frame-styles feature and the DO-LAST compass swap remain. Full detail per item in CHECKLIST.md's matching entry:
...
- [x] Choppy turning — fixed, `d1815e0`. Measured live (not guessed): looking up at the Grand Hall ceiling GLB the first time stalled 40+ seconds — a one-time shader-compile stall (confirmed by an immediate repeat measurement: 53ms for 5 renders) since that GLB loads independently of `populateDynamicContent()`'s existing precompile pass. Now precompiled the moment it's added.
- [x] Grand Hall skylight free-floating lights — fixed, `d1815e0`. 6 (not 4) baked-in `CircleGeometry` discs, confirmed not real lights, removed.
...
- [ ] Museum artwork: 3 selectable frame styles, gated by shelf vs. no-shelf display — NOT started. EK corrected an earlier misread: all 3 already exist in the app — (1) today's plain black/plastic-sleeve frame, restricted to no-shelf wall-hangs going forward; (2) a gold/tan-trimmed white-mat frame seen in-museum on a shelf, source still not located in code after a live-inspection pass; (3) the actual source turned out to be `GalleryShelfScene.tsx`'s `PremiumDisplayCard` (the Exhibitions "Curate the Layout" builder's own gold-gradient-bezel card, not the `/vault` page's `FrameStyle` system as first guessed) — confirmed live via the Exhibitions builder UI. Reuse as-is per EK, do not rewrite. See CHECKLIST.md's entry for the full trail.
- [ ] DO LAST: Grand Hall outer compass floor artwork — NOT started, queued to run after the frame-styles item. Full spec in CHECKLIST.md's entry.

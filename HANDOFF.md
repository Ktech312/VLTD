...
**Code review — gallery sync, deletion, self-healing, polling, error handling.** Read every call site touching these paths (not just the ones added this pass) looking specifically for unawaited writes, ignored `{ error }` results, stale-state overwrites, partial-delete behavior. Found and fixed 4 real defects, all in `d12508f`:

1. **`syncGalleryItemsToSupabase`'s restore-on-failure ran after a conditional throw.** This function deletes all of a gallery's `gallery_items` rows, then re-inserts the new set — not atomic. If the insert failed, a recovery block re-inserts the just-deleted rows... but `if (options?.throwOnError) throw insertError;` fired *before* that block, so any `throwOnError: true` caller (the editor's Save button, the exhibition self-heal — both go through `syncGalleryToSupabaseNow`) skipped the recovery and left `gallery_items` **genuinely empty** for that gallery — the exact table the public share route reads directly. Reordered so restore always runs first.
2. **`deleteVaultItemsEverywhere`'s partial-failure path skipped gallery cleanup entirely**, even for items that successfully deleted. If 2 of 3 mass-deleted items synced fine and 1 failed, the 2 that succeeded would have their ids stuck in `gallery.itemIds` forever, because the function returned early before ever reaching the cleanup call. Now runs cleanup for whichever ids actually succeeded, regardless of partial failure.
3. **`AuctionSetupSheet.tsx`'s start/cancel-auction save was a bare `void upsertVaultItemToSupabase(updated).catch(() => {})`.** Starting an auction sets `isPublic: true`, a reserve price, a buy-it-now — real state a second device or Discover would need to see. A failed sync here was invisible, never retried, and the item stayed live only on the device that started it. Now awaits, queues the item on the existing retry mechanism on failure, and shows a toast instead of silently pretending it worked.
4. **`vault/item/[id]/page.tsx`'s `persist()`** — the single save function behind every edit on the item-detail page (title, value, notes, tags, asking price, video clip; 13 call sites, most of them `void persist(...)`) — had no try/catch around the cloud upsert, which always throws on a genuine failure. An uncaught rejection here also skipped `vltd:vault-updated` entirely (it never got past the throw), so other open views of the same item wouldn't even refresh to the locally-correct state. This is the exact path the "edit item, confirm sync" acceptance check exercises. Now caught, queued for retry, toasted, and the update event always fires regardless of outcome.

Minor, not fixed (flagged, low severity, unrelated to item counts): `upsertGalleryToSupabase`'s invite-token upsert also throws before its own stale-token-pruning step runs — same class of bug as #1, but on `gallery_invites`, a table this pass's testing never touched and isn't part of the count/sync problem.

**Validation suite.** `tsc --noEmit`: clean. `eslint .`: 0 errors outside 27 pre-existing ones confined to `scripts/*.js` build tooling (untouched by this pass). `npm run build`: clean, no errors. No test suite exists in this project — no `test` script in `package.json`, no `.test.`/`.spec.` files anywhere — so "run existing tests" doesn't apply; nothing to run.

**Static layout review — Vault, Add Item, Exhibitions.** Checked every fixed/min-width Tailwind class in `vault/page.tsx`, `capture/page.tsx`, `GalleryBuilder.tsx`, and `museum/[galleryId]/page.tsx`. No confirmed overflow risk: the only page-width-scale fixed widths (Vault's `w-[140px]` stat cards) sit inside an explicit `overflow-x-auto` container that switches to `sm:grid` at the breakpoint; everything else is either a small icon-scale fixed size, a `max-w` cap, or an explicit CSS `min(Npx, calc(100vw - Mpx))` safety pattern already in use in several places. Matches the earlier live no-overflow test.

**Vercel log inspection for the two historical 503s.** Linked the project via `vercel link` (authenticated, correct project). Queried `vercel logs` with `--status-code 503`, then with no filter at all, across multiple windows (6h, unbounded) — **no historical logs retrievable at all**. This project's plan tier appears to only support live `--follow` streaming, not backward-looking queries. This is a genuine tooling/plan limitation, not evidence either way — **status stays "not reproducible on direct retry; cause unconfirmed,"** not upgraded to a confirmed cause just because no logs turned up.

**Discogs / CardHedge / Ticketmaster — env presence + safe connectivity.** `vercel env ls`: both `DISCOGS_TOKEN` and `TICKETMASTER_API_KEY` are present, scoped to Preview+Production. Safe read-only test against the app's own `/api/vinyl-lookup` route (a real, harmless search query, no side effects) confirms **Discogs vinyl lookup is still broken in production right now** — "Discogs token not configured" — despite the var showing as set; almost certainly an empty/invalid value rather than a missing one (didn't inspect the actual value, per instruction not to expose secrets). Not an NYCC launch blocker — vinyl lookup isn't part of the core campaign promise — so this is reported, not fixed. Ticketmaster's only exercisable endpoint is `/api/cron/refresh-events`, which **writes real event rows to production** — declined to trigger it for a "connectivity check," since that's a real, hard-to-reverse action, not a safe test. CardHedge: zero code references anywhere in `src/` — confirmed this was only ever a discussed idea, never built, not a blocker.

**Prepared for the next session**: [docs/nycc-remaining-browser-checks-2026-09-26.md](docs/nycc-remaining-browser-checks-2026-09-26.md) — exact Supabase REST queries (URL, headers, anon key), the disposable-item plan, and precise steps for all seven remaining browser-only checks, so the next session with a working authenticated browser can execute immediately without re-deriving any of this.

**Still not launch-ready.** All seven items from the prior entry's checklist remain open — this pass only closed out what didn't require a browser.
...
**P0 — exhibition item counts (`7/8 Test` gallery, `c1243458-db47-4af1-a0bf-59b7d6bfba0e`), root-caused with live production data, not guessed at.** Queried the actual `galleries` row directly: `layout.itemIds` and `exhibition_layout.itemIds` both held 19 ids. Cross-checked those 19 against a fresh `vault_items` query for the same profile — 10 of them don't exist in Supabase at all. Nothing had ever removed a gallery's reference to an item once that item was deleted; three different delete paths existed and none of them cleaned up exhibitions. Every surface that showed a *different* number was just resolving those 19 stale ids differently (owner card: raw length, no resolution; editor: silently drops anything that doesn't resolve to a locally-loaded item; Guest View: falls back to a frozen `publicItemSnapshots` copy so unresolvable ids still "count"; Discover: its own narrower copy of the id-parsing logic that only checked `layout.itemIds` and never selected `exhibition_layout` at all).

Fixed the actual cause, not the display: `removeItemIdsFromAllGalleries()` (`src/lib/galleryModel.ts`) strips a dead id (and its snapshot) from every gallery, local and cloud. The exhibition editor now self-heals on load — after a real cloud vault-items sync, anything still unresolvable is confirmed dead and pruned everywhere, not just hidden from that one screen. Discover now uses the same canonical id-resolution function as everywhere else instead of its own copy.

**Caught and fixed a real race while verifying this live**: the first version of the self-heal looped the single-id cleanup function once per dead id (10 calls for this gallery), and each call fired its own independent fire-and-forget cloud upsert for the *same* gallery row — confirmed via direct query that the local cache correctly dropped to 9 items but the cloud row stayed at 19 (whichever of the 10 overlapping requests landed last on the server won, not the most-pruned one). Rebuilt it to take every id at once and do exactly one mutation per affected gallery (`removeItemIdsFromAllGalleries`); the same risk existed in Vault grid mass-delete via `Promise.all` and got the same fix (`deleteVaultItemsEverywhere`).

**P0 — item deletion.** Vault grid, item-detail page, and `DropReviewSheet` each had their own delete logic; only the Vault grid's actually deleted the Supabase row. The item-detail page's `deleteVaultItem()` was local-only — a cloud sync or second device could resurrect a "deleted" item. All three (plus mass-delete) now go through one `deleteVaultItemEverywhere()` / `deleteVaultItemsEverywhere()` (`src/lib/vaultActions.ts`) that removes the local copy, deletes the `vault_items` row, and prunes the id from every exhibition. The old local-only `deleteVaultItem()` and `deleteItemAndNotify()` are gone, not just unused.

**Found mid-acceptance-test, not in the original scope, and fixed anyway because it's a real data-loss bug**: created a disposable test item through `/capture` (the main "Add Item" entry point) to run the acceptance script's own item-lifecycle check. It survived a local hard reload — but a direct `vault_items` query by id came back empty. `/capture`'s `handleSave` and `/vault/bulk`'s commit handler both call `appendItems()` (local-storage only) and navigate straight to `/vault` without ever calling `enqueueVaultItemSync()`/`processVaultSyncQueue()` — the step every *other* add path (Scan, manual Add, spreadsheet import, wishlist-to-vault) already does. A new item from the primary Add Item flow could sit local-only indefinitely: invisible on a second device, gone for good if local storage was ever cleared. Both fixed to enqueue + sync immediately after saving, matching the established pattern.

**P1 — mobile access gate.** `ProtectedRoute`'s "Checking access..." was static text with no spinner; the Vault page itself had *no* loading state at all, so a cold load rendered the "Your vault is empty" empty-state for however long the sync took — reading as a broken account, not a loading one. Added a spinner to the gate and a real loading skeleton to Vault (`initialLoadComplete` gate) so the two phases are visibly distinct. Did not attempt to shave the actual network round-trip time (session/profile fetch is already cached after first load; the cold-path cost is one unavoidable REST round trip) — the fix here is honest progress feedback, not a claimed speedup.

**`tsc`/eslint clean on every commit, all pushed to `main` and deployed.**

**Acceptance script — partially run, honestly incomplete, and here's exactly why.** Live-verified with real production data and a mix of UI clicks + direct Supabase/localStorage queries: created a disposable item, confirmed local persistence across a hard reload, deleted it via the item-detail page's modal (not the native-`window.confirm()` Vault-grid one — see below), confirmed it was gone from local storage with nothing to orphan in the cloud. Confirmed the exhibition self-heal actually pruned the regression gallery's stored id list from 19 to 9 locally.

**Not completed, and why**: partway through, Claude-in-Chrome's browser tool became genuinely unreliable in this session — screenshots timing out ("renderer may be frozen or unresponsive"), tabs reporting a 0×0 viewport, and one tab hard-freezing (unresponsive to *any* further command, including plain JS execution) after clicking the Vault grid's delete button, which uses a native `window.confirm()` that this tool has no way to dismiss once it's open. Later in the same pass, typing itself stopped registering in fresh tabs on totally unrelated inputs (the search bar, not just the Add Item form) — a tooling failure, not an app one. Did **not** get through: the 8→6 exhibition item test, cross-viewport (desktop vs 390px) timing capture, or a full console/network/broken-image/overflow sweep across all five flows. The code changes for all of this are shipped and individually verified where the tool allowed it; the *remaining* acceptance-script checks need either a retry once the browser tool stabilizes, or EK clicking through them directly on production, which will be more reliable than further automated attempts risked being right now.

No stray test data left behind — the one disposable item created during this pass was deleted and confirmed gone before wrapping up.

# 2026-09-22 — Museum entry: land on the room-picker map first (live); Task 14 multi-device sign-out fix (live, EK's own cross-device test still outstanding)

Two separate threads, same session, continuing directly from the 2026-09-21 plan below.

**Museum campus first-entry latency — solved a different way than originally scoped, and simpler.** The 2026-09-21 plan below (bake rooms to static files) is still real, useful infrastructure — Museum Builder's "Publish" button, the bake pipeline, and the shared-wall-duplication fix all shipped (`6c7ae84`, `86e27f8`) and work. But wiring the LIVE campus to actually load a baked file instead of building a room turned out to need touching `VltdMuseumCampus.tsx`'s single giant setup effect in a way that risked its cleanup/disposal logic — exactly the kind of risk that file's own history (documented z-fighting bugs, "don't touch `buildSharedWall` casually") warns against. Walked EK through the real trade-off in plain terms (skip-and-load vs. build-then-swap); her own question — "what if you click the room instead of scrolling into it" — was the actual better answer neither of those two options were. Implemented instead (`8371aff`): `/museum/vltd`'s access gate now shows the *existing* `MuseumCampusOverview` room-picker map first when there's no `?room=` param, and only renders `VltdMuseumCampus` once one's chosen (clicking a room on that map already linked to `?room=<id>`, which `VltdMuseumCampus` already read to pick its spawn point — neither file needed a single line changed, the gate just wasn't using them together). **Live-verified**: clicked POP_CULTURE from the map, landed in it fully interactive within ~1.2s (real synchronous-setup timing from the still-active debug logs), console-confirmed via the existing `[perf]` instrumentation. Honest caveat: the underlying full-campus synchronous build cost is UNCHANGED by this fix — it still runs in full, still varies 1-7+ seconds by JS-engine warm state per the 2026-09-21 measurements. What changed is that it now happens in direct response to a deliberate click instead of automatically on page load, which is what actually made it read as broken. The baking work from 09-21 remains the real fix for the underlying cost itself, whenever room-by-room conversion resumes.

**Task 14 — multi-device sign-out bug, found live by EK mid-session** (commit `8a95a06`): while testing the above, EK hit exactly the bug logged as Task 14 in VLTD-Dev-Tasks.md the day before — repeatedly asked for a fresh 2FA code despite already being logged in on the same browser. Root cause confirmed: every `supabase.auth.signOut()` call in the app was unscoped, which defaults to Supabase's own "global" scope — any ordinary sign-out anywhere (including the "sign out instead" escape hatch on a stuck 2FA prompt) was silently revoking every session on the account, not just the one that clicked it. Fixed per EK's own exact spec: `src/lib/auth.ts`'s `signOut()` now defaults to `{ scope: "local" }` (every existing call site — TopNav's Sign Out, admin pages — correct with zero change); `src/lib/adminAuth.ts` and `MfaChallengeGate.tsx`'s `signOutInstead()` hardcoded to local. **One real snag caught before shipping**: Account → Security already had a working "Sign out all" / "Sign out everywhere" button that called this exact same shared function — defaulting it to local would have silently broken that button's entire purpose. Its one call site now explicitly passes `signOut({ scope: "global" })` instead. Confirmed via full-repo grep that these are the only 4 `supabase.auth.signOut()` call sites anywhere — no unscoped call left. 2FA/admin protection itself untouched.

`tsc`/lint/build all clean on both threads. **Not done**: EK's own real desktop+phone side-by-side test for Task 14 (needs her actual two devices, can't be run from here) — she asked for each of the 5 sub-checks reported separately once run, not a blanket "verified."

# 2026-09-21 — Scoped plan: bake museum campus rooms to static files instead of building them live in every visitor's browser (NOT YET IMPLEMENTED — plan only, EK's go-ahead needed before Phase 1 starts)

Follows directly from the 2026-09-17-ish "well over 10 seconds to get into the main hall" root-cause entry below (same investigation, same session thread). EK asked "why wasn't it scoped this way from the beginning" and "can I still customize rooms individually from the backend" — both answered, then asked for a concrete plan. Researched the actual code (not guessed) before writing this:

**Why it's procedural today, for the record:** Museum Builder (the existing admin tool) saves room settings (`museum_room_meta`: style/capacity/wallpaper/frame style/item placement) to Supabase, and the live campus reads those settings and reconstructs the room from scratch, live, in every visitor's browser (`VltdMuseumCampus.tsx`'s one giant `useEffect`, `src/lib/campusRoomBuilder.ts`'s `buildRoomShell`/`buildRoomTrim`/etc.). That means any Museum Builder edit shows up for every visitor instantly with zero extra step — a reasonable trade while the rooms themselves were still being actively hand-corrected (this file alone documents dozens of correction passes to that same builder code). The cost: every visitor pays the full construction cost themselves, every visit, which doesn't hold up at scale.

**The plan — keeps Museum Builder as the only editing surface, adds a bake step behind it:**

1. **Bake mechanism — client-triggered, no new server infra needed.** `MuseumBuilder.tsx` already runs its own live `WebGLRenderer`/scene to preview a room while editing (same builder functions as the live campus). Add a `GLTFExporter().parse(scene, ...)` call on that already-built scene, upload the resulting `.glb` via `supabase.storage.from("museum-room-bakes").upload(...)`, save the returned URL to `museum_room_meta.baked_asset_url` (+ `baked_at` timestamp). No server-side Three.js needed — confirmed none exists in this repo today, and the only existing async-job pattern (`api/cron/*`) has no rendering capability to build on, so client-side export is the actually-simpler path, not a shortcut.
2. **New explicit "Publish" action in Museum Builder**, separate from its existing continuous autosave. Today there's no single Save — every field (capacity, layout, background, style, frame, item placement) autosaves independently and immediately. Baking on every keystroke-adjacent autosave would thrash and could publish half-finished edits to visitors mid-edit. A dedicated Publish button lets EK finish arranging a room, preview it, then explicitly push that version live — this is a real design improvement on top of the perf fix, not just plumbing, and needs EK's sign-off on the UX before building it (alternative: auto-bake on a long debounce with no button — flagging both options, recommending the button).
3. **Shared walls solved by small, deliberate duplication, not a cross-room dependency.** `buildSharedWall()` builds ONE mesh per shared room boundary with two different face materials (room A's finish one side, room B's the other) — confirmed via `campusRoomBuilder.ts:963`. Rather than trying to bake walls as some third shared unit (real cross-room dependency, real risk given this is the single most-corrected, historically z-fighting-prone geometry in the codebase per its own in-file history comments), each room's own baked file includes a copy of every wall segment bordering it, built exactly as today. A shared wall exists once in each of its two neighbors' files — a handful of extra Box meshes per file, nowhere near the actual cost driver (furniture/trim/cases), in exchange for every room's bake being fully self-contained and independently loadable.
4. **Schema**: extend existing `museum_room_meta` (not a new table — it's already the one-row-per-room upsert target for every other per-room override) with `baked_asset_url text`, `baked_at timestamptz`. Needs adding to `ROOM_META_COLUMNS_FULL`'s column-cascade list in `museumCampusConfig.ts` (the existing fails-soft pattern for schema drift) or it'll silently be dropped on read.
5. **New Storage bucket** `museum-room-bakes`: public (visitors need no auth to load a baked room), write-gated to admins matching `museum_room_meta`'s own RLS pattern (`user_roles` table check) rather than the per-user `auth.uid()` folder pattern `room-wallpapers` uses — this is shared admin-curated content, not per-user content, so that's the closer precedent to copy.
6. **`VltdMuseumCampus.tsx` restructure — the actual visitor-facing fix.** Replace the one-giant-synchronous-`useEffect`-builds-everything shape with: on mount, determine the spawn room (`?room=` param or default), fetch and `GLTFLoader`-load ONLY that room's baked file, mark the page interactive as soon as it's in the scene (not after the whole campus). Then, in the background (`requestIdleCallback` / chunked), preload the spawn room's immediate neighbors using the *already-existing* `adjacentRoomIds()`/`CAMPUS_DOORS` graph (`campusLayout.ts:147,215` — this doesn't need to be built, it's already there and already used for lighting activation) so walking through a doorway feels instant. Clicking a waypoint/teleport target that isn't loaded yet loads it on click with a brief transition. **Fallback for rooms with no baked asset yet** (`baked_asset_url` null): fall through to today's procedural builder for just that one room — this is what makes an incremental, room-by-room rollout possible instead of a big-bang cutover of the whole campus at once.

**Rollout order, deliberately conservative:** convert ONE room end-to-end first (bake button → storage → loader → live verification with EK watching), matching this codebase's own established caution for this exact file ("room-shell-type by room-shell-type... not one giant sweeping change," from the earlier draw-call merge work in this same file) — the wall/shared-geometry code here has a real documented history of subtle, oblique-angle-only visual bugs. Only extend to the rest of the campus once that one room is confirmed correct from every angle.

**Not started.** Waiting on EK's go-ahead on Phase 1 (one room, full pipeline) before writing any code, and specifically her call on the Publish-button-vs-auto-bake-on-debounce question in point 2.
...
# 2026-09-19 — Private Photos (paid feature), built end to end; also fixed real hardware zoom silently never activating on any device

**Private Photos.** EK's own long-flagged direction ("free stays public, paid gets real privacy") had no architecture decision behind it — this session walked through the trade-off directly with EK before building anything. Two options on the table: (a) rewrite every image in the app to use temporary signed URLs, or (b) a second, opt-in bucket a specific photo migrates into only when a paying user actually marks that item Private. Went with (b) — not just safer to ship (zero risk to anything that already works, since free accounts and every existing photo never touch any of this new code), but genuinely the better architecture at real scale too: (a) would add non-cacheable request overhead to every photo view for every user, paid or not; (b) keeps the overwhelming majority of photos on the existing CDN-cacheable public path forever, and only pays the signed-URL cost for the narrow slice someone actually locks down.

Built:
- **Migration `20260919_private_photos.sql` — ✅ confirmed run by EK 2026-09-19.** New `vault-images-private` bucket, not public. Upload is gated in Postgres on the caller belonging (owner/admin/member — `profile_members` covers all three) to a profile with `tier <> 'FREE'` — a **real server-side check**, deliberately not trusting the client's own `getTierSafe()` cache (already flagged elsewhere in this file as tamperable via devtools — this is exactly the kind of check that can't be allowed to rely on it). Read/delete only check folder ownership, not tier, so an already-private photo stays private and manageable even if a subscription later lapses.
- **`src/lib/privatePhotos.ts`** (new) — `canUsePrivatePhotos()` (real `profiles.tier` read), `migrateItemImagesToPrivate`/`migrateItemImagesToPublic` (move one item's images between buckets on demand — download current bytes, re-upload to the other bucket, remove the old copy), and `resolvePrivateImageUrlSync()` — a synchronous signed-URL cache with async backfill, using this app's own existing `vltd:vault-updated` event to trigger re-renders once a real signed URL lands. This is the piece that made the whole feature tractable without a huge blast radius: `getPrimaryImageUrl()`/`resolveVaultImageUrl()` are called synchronously from ~28 places across the app (every vault card, gallery, museum room), and none of them needed to change — `resolveVaultImageUrl()` just checks the image's own `isPrivateStorage` flag and routes to the cache instead of the public-URL resolver.
- **`VaultImage` gained `isPrivateStorage`**, round-tripped through `vaultCloud.ts`'s existing read/write mappers (`rowToVaultImage`/`sanitizeRemoteImages`).
- **`ItemVisibilityToggle.tsx`** — the existing Public/Private toggle now does the migration as part of the same click: going Public → Private moves images to the private bucket only if the profile is actually paid (checked live, not from cache); going Private → Public always moves them back, regardless of current tier, so a downgraded account can still make an item public again.

**Not yet live-verified end to end** — needs a real paid test account: mark an item Private, confirm the photo actually moves buckets and still displays (via the signed-URL cache) for the owner, confirm a signed-out/different-account view of that item's photo genuinely fails now.

**Also fixed, same session: real hardware/optical zoom was silently never activating, on any device.** Flagged as "never verified on a real phone" — a code review before handing it back for real-device testing found the actual bug: `useCameraZoom.ts` checks `track.getCapabilities().zoom` to decide whether to offer real optical zoom, but Chrome only ever populates that capability if the *original* `getUserMedia()` call explicitly requested `zoom: true`. Neither camera panel (`ScanCapturePanel.tsx`, `CameraCapturePanel.tsx`) ever included it, so the app was silently always falling back to digital crop-zoom — not broken/crashing (the digital fallback is correctly implemented and produces a real matching photo), just quietly never using real optical zoom even on phones whose hardware supports it. Added `zoom: true` to both panels' primary `getUserMedia` request (not `CameraCapturePanel`'s plain `video: true` fallback, which is deliberately unconstrained as a last resort). Still needs a real phone to confirm optical zoom now actually engages where hardware supports it. The other two real-device-only items from that same review — ultra-wide lens auto-switching (label-text heuristics, genuinely can't be verified without real device labels) and the compact touch-only camera-picker icon (code-reviewed, uses `matchMedia('(pointer: coarse)')` correctly, no issue found) — are unchanged, still waiting on EK's own phone.

# 2026-09-17 — Friendly Glass nav: real 32px size increase, header height untouched; live and DOM-verified on production

Follows the crop-fix entry below (same thread, `bcb79e5`). EK's icons were still too small even after the tight crop — her instruction: the icon can fill the tab's square and the label can shift down, as long as nothing else changes size, with an explicit standalone follow-up: **"DO not make the HEADER larger."**

**Commit `8e383ac` (live)**: changed the `AppIcon` `size` prop from 20px (TopNav) / 22px (BottomNav) to **32px** on every nav tab (both navs' regular tabs and each "More" trigger; BottomNav's center capture "+" button untouched) — a real layout-box size change via the prop, not a CSS `transform: scale()` trick (that approach was tried and reverted earlier this session, `49cc8d0`, because it didn't reflow label spacing correctly). Recalculated padding/gap so the bigger icon+label still fit the existing containers without growing them: TopNav tab `gap-[6px] px-3 pt-2 pb-[10px]` → `gap-[4px] px-3 pt-1 pb-2`.

**Verified, not assumed:**
- `tsc --noEmit` clean, targeted ESLint 0 errors (1 pre-existing unrelated warning), production build clean.
- Live on `vltd.vercel.app/vault` (authenticated session), direct DOM measurement via `getBoundingClientRect()`/`getComputedStyle()` — not a screenshot: header `65px` before and after (genuinely untouched), tab box `59px` fits inside with `2.5px`/`3.5px` clearance top/bottom, friendly-glass `<img>` renders at a real `32×32` box.
- Zoomed screenshot of the live TopNav confirms all 6 icons (Vault, Exhibitions, Discover, Events, Insights, More) are now clearly recognizable at native size, header bar visually unchanged.
- BottomNav (mobile-only, `md:hidden`) carries the identical `size={32}` change on the identical `AppIcon` component/logic already proven live above, and has no fixed-height container to begin with (just row padding, confirmed by reading its wrapper styles) — so there's no header-growth risk there either. Could not re-confirm with a live mobile-width screenshot this session: this environment's browser window is a fixed-resolution virtual display and does not actually resize despite `resize_window` reporting success (window stays 1920px wide; documented, unresolved tooling gap, not re-litigated further). Was visually confirmed at a real mobile viewport (375×812) earlier in this session before this final commit, showing clean, non-overlapping, properly-spaced larger icons.

# 2026-09-16 (latest) — Friendly Glass nav: reverted the size fix, tight-cropped all 109 icons instead; live, EK's own call on legibility still pending

Follows directly from the two entries below (same day). Sequence of real mistakes and corrections, in order:

1. First attempt (commit `52d3fb5`) scaled nav icons up to ~32px via CSS transform to fix legibility. EK rejected this on sight — it threw off icon-to-label spacing and she does not want the nav's own proportions changed, full stop.
2. While investigating, sent EK screenshots claiming things looked fine — she correctly called this out (a screenshot re-shrinks the whole page, so it's not reliable evidence of what a 32px icon looks like on a real screen, and it isn't reliable evidence something's broken either). Built a review Artifact linking directly to `vltd.vercel.app`'s icon files instead of embedding them — Claude's own artifact sandbox silently blocks images from external hosts, so EK saw a wall of broken-image icons. Real, disclosed, self-inflicted bug. Rebuilt with the images base64-embedded in the page and re-verified rendering before sending back.
3. **Commit `49cc8d0` (live)**: reverted the size transform entirely — TopNav is back to exactly 20px, BottomNav exactly 22px, matching Classic, no layout size change anywhere. Real work this round: `scripts/crop_friendly_glass.js`, a `sharp`-based batch tool using `.trim()` (alpha bounding box) to remove each icon's transparent margin and re-center the art on a square canvas, run across all 109 icons in both `design-assets/` and `public/`. Before: icons filled 79-97% of their frame, inconsistently, per icon. After: every one of the 109 fills 92.4-92.7% — genuinely tight and finally consistent (addresses "they are all inconsistant" directly). Also increased the icon-to-label gap on both navs (TopNav 3px→6px, BottomNav 2px→5px) since even at native size the label sat too close to the icon — a pure spacing fix, applies to Classic too, no size change.
4. **Found, disclosed, not fixed**: several icons (vault, exhibitions, discover, notifications, others) carry a small sliver of an adjacent icon bleeding in from one edge — a pre-existing defect from how the 7 source sheets were cut into individual cells, present before any of this session's changes. It's opaque bled-in content, not transparent padding, so cropping can't remove it; a real fix means re-cutting the sheets at the source, not attempted here.
5. Published a final comparison Artifact (same URL as the earlier ones, "Nav Fix — Real Size Check") with everything embedded (learned from mistake #2): TopNav and BottomNav at their real unchanged sizes with the new spacing, the same 6 icons at 5x magnification, and a real before/after of the crop on the two worst-padded icons (More, Add/Capture) plus two already-good ones (Vault, Exhibitions) for a fair comparison.

Verified: `tsc --noEmit` clean, targeted ESLint 0 errors, production build clean, Classic confirmed pixel-identical (no spacing/size regression), nav icons confirmed back to native 20/22px with no transform via direct DOM measurement on the live site. **EK's own judgment on the artifact is the real gate here** — her closing instruction was explicit: "if i still cant read them, then they get scrapped." Not yet confirmed either way as of this entry.

# 2026-09-16 (later) — Friendly Glass navigation: NOT accepted, rollout paused, isolated review preview built instead

EK reviewed the shipped navigation treatment (commits `1ff5152`/`78d7fe3`, below) and rejected it — the opacity-dimmed inactive icons at TopNav's 20px / BottomNav's 22px did not read as intended. Per her explicit instruction, **no further production nav changes were made this round**; the live site is unchanged from the entry below. Instead, built and published a standalone review Artifact (`Friendly Glass Nav Review`) so the open questions can be judged from screenshots before anything ships:

- **Padding, measured not guessed**: wrote a canvas alpha-bbox script against all 7 nav-relevant PNGs. Five of seven (Vault, Exhibitions, Discover, Events, Insights) already fill 85-97% of their 256x256 canvas — the "tiny icon" complaint isn't the source art for those. Two are genuinely under-filled and off-center: **More** (80% vertical fill) and **Add/Capture** (79% vertical fill), both padded low in the frame. The preview shows "as shipped" vs. a tight-cropped version (same source pixels, re-centered, proportions preserved) for all 7 side by side, toggleable against both a dark and a light nav background.
- **Active-state treatment**: recreated TopNav's real existing underline bar and BottomNav's real existing pill+border+gold-dot — both already shipped, already accepted, doing the "surrounding highlight" job today in Classic — instead of inventing a new mechanism. The candidate treatment drops the icon-level opacity dimming entirely (inactive icons full opacity; only the label color dims, same as Classic already does) and lets those existing surrounding cues carry the active/inactive read.
- **Full header mockups** at real desktop TopNav width and real 390px mobile BottomNav width, with a live 48px/56px toggle, using the real background colors/spacing/fonts from `TopNav.tsx`/`BottomNav.tsx`. Utility icons (search, chat, bell) stay Classic and small, unchanged, per instruction — only the 5-6 primary destination icons scale up.
- **One bug caught in this session's own "look once" pass before publishing**: the BottomNav mockup's `max-width:100%` let the 390px-wide mock shrink inside a narrower viewport, and the 5 flex children overlapped instead of scaling down cleanly. Fixed by making the mock a fixed 390px inside a horizontally-scrollable wrapper (matches the "real mobile width, no scaling" premise anyway) — re-verified clean before publishing.
- No `AppIcon.tsx`/`ThemeContext.tsx`/`ThemePicker.tsx` changes this round. The global theme-switch infrastructure from `1ff5152`/`78d7fe3` is untouched and still live, still defaulting to Classic in production.
- **Authenticated live mobile-viewport verification (BottomNav, Vault favorites, Messages star, Account picker, an action modal) — still not done.** Claude-in-Chrome's window resize reports success but `window.innerWidth` stays at 1920 in this environment (confirmed again this session, not re-litigated further); no second device is paired; copying the session's `localStorage` auth token into a viewport-emulating browser was considered and explicitly rejected — that's exactly the kind of credential/token handling this session doesn't do, moving auth material between browser contexts even for the user's own already-logged-in session. Flagged to EK directly rather than worked around; her call on how to proceed once the nav treatment itself is settled.

# 2026-09-16 — Friendly Glass icon theme: switchable 3D icon pack shipped behind Classic default, verified live on production

Unrelated to the museum/2FA work above. Completes the icon-centralization work from `020adf2`/`3f0d297` (the single `AppIcon` component covering nav + the full app sweep) by installing a 109-icon "Friendly Glass" 3D/glass pack as a second selectable style, per a design-chat brief (`design-assets/icon-themes/friendly-glass-v1/CODER-HANDOFF.md`). Three commits, all live: `1ff5152` (initial integration), `78d7fe3` (two real bugs found and fixed in this session's own verification pass, before EK ever saw it), plus this session's live-production verification with no further code changes needed.

**What shipped** (`1ff5152`): `AppIcon.tsx` renders both a classic SVG layer and a Friendly Glass `<img>` layer for every icon; `data-vltd-icon-style` on `<html>` (plain CSS, no client hook inside `AppIcon` itself, so it stays usable from server components) picks which one shows. `ThemeContext.tsx` gained `iconStyle`/`setIconStyle`, persisted to `localStorage` (`vltd_icon_style`) via the same hydration-guard pattern already used for the background theme — confirmed independent of it in both directions. `ThemePicker.tsx` (the existing appearance menu, reachable from TopNav and Account) got a new "Icon style" section: Classic / Friendly Glass, each option previewing its own art regardless of which is globally active. Classic is the default; all 109 `AppIconName` entries verified 1:1 against `public/ui/icon-themes/friendly-glass-v1/*.png` (scripted check, no missing/orphaned names).

**Size floor, verified not guessed**: the pack's own manifest calls out 40px+ as its strongest size, but most of this app's icons render far smaller (TopNav 20px, BottomNav 22px, functional badges as small as 9px). Built a disposable pixel-accurate test harness (native render vs. a 4x magnification of the same downscaled bitmap, 8-48px) rather than trust a first look at a compressed screenshot, which was misleadingly harsh — the real threshold measured legible down to ~18px. `AppIcon.tsx` hard-codes `FRIENDLY_GLASS_MIN_SIZE = 18`: any icon requested smaller renders Classic only, regardless of the global setting, no per-page exceptions.

**Two real bugs found in this session's own verification, fixed in `78d7fe3` before reporting anything done:**
1. **`filled` (saved/liked toggle) had no Friendly Glass treatment at all** — the pack is one fixed asset per name, so a favorited star and an unfavorited one looked identical. Added a gold ring + small checkmark badge on the tile when `filled=true` and Friendly Glass is active (via a `.vltd-icon-friendly-badge` class gated the same way as the classic/friendly toggle itself), applied through the shared component so every `filled` consumer (FavoriteButton, wishlist hearts, the Messages star toggle) gets it automatically. Classic is untouched.
2. **Real regression: any icon below the 18px floor rendered as NOTHING when Friendly Glass was active.** The classic `<svg>`/wrapper `<span>` always carried the `vltd-icon-classic` class; the global CSS rule hiding that class whenever Friendly Glass is active doesn't know about the per-instance size floor, so with the friendly `<img>` correctly suppressed below 18px AND the classic layer also hidden, small icons vanished — caught via a disposable dev route rendering both filled states at multiple sizes, not caught by the earlier pass's spot-checks. Fixed by only applying `vltd-icon-classic` when the friendly layer is actually being rendered (`size >= 18`); below that the classic layer carries no toggle class and is always visible.

**Live-verified on production** (`vltd.vercel.app`, real authenticated Chrome session, not localhost): TopNav, Vault (grid + item detail), Exhibitions, Museum Builder, Messages (including the small below-floor star toggle and its Classic fill behavior), Account Settings, and a real action modal (Social Export) — Classic → Friendly Glass → Classic via the actual Theme Picker UI, confirmed instant with no layout shift, confirmed a hard page reload preserves the selection (`localStorage`/`data-vltd-icon-style` both read back `friendly-glass` after reload), confirmed switching Background (Graphite → Gunmetal) does not reset Icon style and vice versa, zero image 404s across every navigation. **Not verified**: a live authenticated mobile-width (390-414px) pass — Claude-in-Chrome's `resize_window` reported success but `window.innerWidth` stayed at 1920 throughout this session on this machine, a tool limitation, not a product one. Mobile TopNav/BottomNav rendering at 375×812 was confirmed correct in an earlier pass of this same work via the sandboxed preview pane on an unauthenticated build (same code path, same nav icon sizes) — that check doesn't cover the new filled-badge feature specifically at mobile width. A live favorited-star instance in a public gallery (FavoriteButton via `GalleryLayout`/`GuestGalleryRenderer`) was not hunted down on production; the filled-badge logic itself was verified thoroughly via the disposable dev route across every size/variant combination instead.

`tsc --noEmit` clean, targeted ESLint 0 errors (3 pre-existing `@next/next/no-img-element` warnings — `next/image` isn't compatible with the dual-layer CSS-toggle approach, a deliberate choice), `npm run build` clean on every route both times. Vercel production status confirmed `Ready`/`success` for both commits via the GitHub commit-status API.

# 2026-09-14 — 2FA lockout: EK's own root-cause report, fixed, confirmed back in; 2 items deferred to next batch

**Unrelated to the ceiling/museum work below — EK's own investigation traced this to the global 2FA gate (`MfaChallengeGate.tsx` + `Providers.tsx`, added 2026-09-12), explicitly ruling out any ceiling commit.** Full detail in CHECKLIST.md's matching entry (top of that file). Three commits: `91d0d7a` (made `VltdMuseumAdminGate.tsx`/`MuseumBuilderOwnerGate.tsx`/the museum Builder-button check distinguish "not an admin" from "admin, 2FA just not done yet this session" instead of a hard "Not authorized" that never recovered — plus a same-commit attempt at reducing unwanted modal re-challenges) → `78cad6a` (that re-challenge fix was a real regression — EK reported being unable to reach the modal at all; reverted it, and separately root-caused a genuinely pre-existing bug found while investigating: `globals.css`'s `body > * { position: relative; z-index: 1 }`, written for noise-layer stacking, silently overrides the modal's `.fixed`/`z-[999999]` classes because Tailwind v4's utilities live in `@layer utilities` and unlayered CSS always beats layered CSS regardless of specificity — the modal has likely never rendered as a real overlay since it was built) → `df10d33` (position was fixed but the input still wasn't clickable — same rule also stomps z-index, which the first override didn't touch; added a second scoped override). **EK confirmed live: back in, modal clickable, verify works.**

Ten items EK explicitly asked to defer to "the next batch of fixes" (the last one queued explicitly to run AFTER the rest). Status after this session's continuous work-through-the-list pass — 9 of 10 fixed and deployed (commits `a99c321`, `fa51153`, `77eff75`, `d1815e0`, `84d4102`, `7300de0`); only the frame-styles feature's 3rd style remains unlocated. Full detail per item in CHECKLIST.md's matching entry:
- [x] 2FA modal Enter-key submit — fixed, `a99c321`.
- [x] Doorway stuck-collision — fixed, `d1815e0`. Root cause found without needing EK to name a door: `moveWithCollision()` stopped outright on any combined (x,z) step that left walkable ground, and a doorway's own walkable "bridge" is exactly the door's visual width with zero side margin — now slides along whichever axis is still open.
- [x] Choppy turning — fixed, `d1815e0`. Measured live (not guessed): looking up at the Grand Hall ceiling GLB the first time stalled 40+ seconds — a one-time shader-compile stall (confirmed by an immediate repeat measurement: 53ms for 5 renders) since that GLB loads independently of `populateDynamicContent()`'s existing precompile pass. Now precompiled the moment it's added. Live-reverified after the compass swap deployed: instant render, no stall.
- [x] Grand Hall skylight free-floating lights — fixed, `d1815e0`. 6 (not 4) baked-in `CircleGeometry` discs, confirmed not real lights, removed. Live-reverified: the one remaining ceiling light visibly hangs from the skylight's own ridge beam — a different, already-supported fixture, not a residual instance of the same bug.
- [x] Museum doorway nav targets: chevrons — fixed, `fa51153`, confirmed live via screenshot.
- [x] Museum artwork wall-seam — fixed, `77eff75`. Root cause measured live: the standard wall-art stand-off was shallower than Vault-armor's own divider/rivet protrusion on that wall.
- [x] Museum artwork labels: gold-plaque style — fixed, `77eff75`. Reuses `buildDestinationSign()`'s own plaque construction at item scale.
- [x] Museum entry corridor (PLAZA) marble/wall/sky — fixed, `d1815e0`. Confirmed "Corridor" HUD label is PLAZA's own fallback text.
- [x] Museum artwork: 3 selectable frame styles — 2 of 3 fixed, `7300de0`. "classic" (today's look) and "gallery" (gold-gradient bezel, confirmed live as `GalleryShelfScene.tsx`'s `PremiumDisplayCard` from the Exhibitions "Curate the Layout" builder — not the `/vault` page's `FrameStyle` system as first guessed) are real and selectable per room in Museum Builder, next to the existing style/capacity controls (EK's own answer to "where should this live"). Persisted via `museum_room_meta.frame_style` — **new migration, not yet run by EK**, SQL given to her directly. Shelf items always get "gallery," enforcing the no-shelf-only rule for "classic" automatically. The 3rd style (gold/tan-trimmed white-mat frame seen in-museum on a shelf) is still unlocated in code — flagged, not built.
- [x] DO LAST: Grand Hall outer compass floor artwork — fixed, `84d4102`, done last. Approved PNG copied from EK's main checkout, SHA-256 verified, swapped in at the same footprint; center logo untouched. Confirmed live.
...
# 2026-09-13 (POP_CULTURE Vault-parity pass) — wall armor + display cases/shelves wired into the LIVE public campus for the first time, generically across every editable room

Work order: `docs/POP-CULTURE-VAULT-PARITY-2026-09-13.md` — this doc's own preamble discloses it is an interpretation of EK's overnight instruction ("use the source-checked POP CULTURE instructions above"), since no such prior instructions were found anywhere in this session's transcript; built against the doc's own code-verified interpretation instead, and EK may redirect in the morning. Closes the exact "noted, not done" follow-up left at the bottom of the 2026-09-12 Vault-style full-parity entry directly below: that pass wired real armor/furniture into Museum Builder only; this pass wires the SAME already-existing functions into `VltdMuseumCampus.tsx` (the real walkable campus), generically, not a POP_CULTURE-only special case.

**Database check (read-only script, deleted after use, same pattern as the earlier MFA-fix session): migration `20260912_museum_room_style.sql` HAS been run** — `museum_room_meta.room_style` exists and POP_CULTURE's own row already has `room_style = "vault"` saved. So POP_CULTURE's wall/floor/ceiling/baseboard/rail materials and light rig should already have been resolving to real Vault materials live, via the style-patch loop that already existed before this pass — the gap was never that mechanism, it was armor + furniture never being called from this file at all.

**What changed (3 files: `VltdMuseumCampus.tsx`, `src/lib/campusRoomBuilder.ts`, `src/lib/museumRoomFurniture.ts`):**
- **Wall armor**, `museumRoomArmor.ts`'s `addStyledRoomArmor()` (built and proven in Museum Builder on 2026-09-12, never called from the live campus until now) — called once per `EDITABLE_ROOM_IDS` room inside the existing style-patch loop, using this room's own real wall segments (`computeCampusWallSegments()`, already computed in this file) and its own real saved `room_style`. Unconditional per room: the function's own internal dispatcher already no-ops for anything other than `"vault"`/`"loft"`, so a room with no style is completely unaffected.
- **Display cases + shelf boards**, `museumRoomFurniture.ts`'s `buildDisplayCase()`/`buildShelfBoard()`/`createShelfMaterial()` (also Museum-Builder-only until now) — built per room from that room's own real `museum_room_meta.case_capacity`/`shelf_capacity` (reusing `computeRoomCaseSlots()`/`computeRoomShelfSlots()`/`computeRoomShelfSpans()` from `campusRoomBuilder.ts`, exactly as Museum Builder already does), independent of `room_style` — a room with no style still gets real furniture using that pair's existing plain-material fallback. `case_capacity`/`shelf_capacity` null-or-0 both mean "off," per that column's own documented meaning (`20260912_museum_room_capacity_and_background.sql`'s comment) — unchanged. One additive default on top: a room that DOES have its own saved style but has never had `case_capacity` set at all (genuinely null/undefined, not a stored 0) defaults toward the personal Gallery Vault's own real 5-case layout (`CABINET_SPOTS`, `galleryRoomSlots.ts`) instead of silently showing none — gated on the room having a style so an unstyled room with no capacity row (e.g. TCG/COLLECTION today) stays at exactly zero cases, matching "any room with no saved style must render exactly as before."
- **Curated-item placement** now folds each room's own case/shelf slots into the existing wall-slot assignment pass (`buildSlotAssignments`) — wall slots stay first in the combined list, in the same order as before, so a room's existing wall-only assignment is untouched; case/shelf slots only ever pick up items that would otherwise have gone unplaced. Case items place via `placeItemsInCases()` (already exported, previously Museum-Builder-only caller).
- **Item interaction**: confirmed (by reading, not by watching) that the live campus has NO click/hover info-popup system at all today — its pointer/raycaster handling only ever targets floor waypoints for navigation. The real "info treatment" every wall/shelf item already gets is a baked, always-visible compact title plaque (`hangCompactLabel`, `campusRoomBuilder.ts`) under the item — case items previously got none at all, even in Museum Builder. Extended the SAME treatment to case items: `hangCompactLabel` exported (was module-private), `placeItemsInCases()` gained an optional `placeLabel` callback (so Museum Builder's own existing call site is unaffected — it simply doesn't pass one) that the live campus now uses to mount a small title plaque on the front (room-facing) side of each case. `computeRoomCaseSlots()`'s case-slot `rotationY` (previously a hardcoded, never-read `0`) now carries the real outward-facing rotation for this — confirmed dead until this pass (`buildDisplayCase` never read it; `placeItemsInCases` only ever applied its own fixed horizontal rotation), so this is a zero-effect-on-existing-callers change.
- **Wall-armor visibility — still genuinely open, not resolved this pass.** The 2026-09-13 fix commit `2e20446` (directly below) corrected the wall-face offset math; re-verified this pass that `buildSharedWall()` really does center each wall box on `segment.fixed` ± `wallThickness/2`, confirming that fix's premise is sound. But the doc disclosed that even after that fix, a live check in Museum Builder STILL showed no visible panel dividers/rivets, and no further fix commit exists after `2e20446`. **This session has no browser/visual verification capability at all** — did not and could not check whether armor is actually visible once rendered, in Museum Builder OR the live campus. What WAS verified: a scratch script (deleted after use) called `addStyledRoomArmor` directly against POP_CULTURE/TCG/COLLECTION/SPORTS/CARDS's real geometry — it throws no errors and adds a plausible mesh count (86-92) per room, with panel/divider counts that shouldn't collapse to zero for POP_CULTURE's own real wall spans (its north wall alone is 21 units — 4 panels, 3 interior dividers at `VAULT_PANEL_WIDTH=4.9`). That rules out "panel-count rounds to 1" as POP_CULTURE's own failure mode specifically, but does NOT prove the geometry is actually visible/well-lit/contrasted once rendered — that remains unverified and open, exactly as the work order left it. Flagging for the parent session/EK: dig into it with a real render before calling armor "fixed."
- Confirmed via diff: `campusLayout.ts` (room geometry/doors/doorways/nav waypoints/walkable areas), `MuseumBuilder.tsx`, `VirtualGalleryRoom.tsx`, `galleryRoomFinishes.ts` all byte-for-byte unchanged — only reused their exports. HUB/Grand Hall block in `VltdMuseumCampus.tsx` (~line 541-622 at the time this branch started) untouched — a separate background agent was concurrently working that block; rebased onto its landed changes before pushing (see commit list below) and kept its HUB-scoped lines untouched next to this pass's own changes elsewhere in the same file.
- `npx tsc --noEmit` clean (0 errors) / targeted ESLint on all 3 changed files — 0 errors, 1 pre-existing unrelated rule-suppression warning carried over (same one every prior pass in this file has noted) / `npm run build` — clean, exit 0 (ran a real `npm install` first; this worktree had no installed dependencies, same as every prior pass's own note).
- **Not live-verified — cannot be, this session has no browser.** Do not treat a clean build as evidence of a correct visual result. The parent session/EK need to actually load `/museum/vltd`, stand in POP_CULTURE, and compare it against the real personal Vault room before this is "done" or "matching the Vault."
...
Verified live against production with real interaction (click/tap-select, drag-and-drop via the keyboard Move-menu path, Replace and Cancel, remove, re-add, keyboard Tab+Enter+focus-return, desktop/tablet/tablet-landscape/phone viewports) using synthetic local test items (no real vault/Supabase session was available to this session — Claude-in-Chrome would not connect, and the sandboxed preview browser has no login). **Not verified this session, needs EK or a real signed-in pass:** the actual Supabase Save Hall / autosave round-trip and Hall rename (both need a real profile), a saved Hall's reload-persistence, the Map showing more than one real assigned Hall side by side, and physical touch (press-and-hold specifically) on real hardware. Desktop-only native HTML5 drag-and-drop could not be driven by this session's browser-automation tool (a known limitation of simulating that specific gesture) — the identical underlying move/replace logic was proven correct via the keyboard Move-menu path instead, which calls the exact same function.

Do not describe this as accepted — EK's own review is still the real acceptance gate, same as everything else in this file.

---

This is the personal Gallery Builder at `/museum/virtual-room`, not the public walkable campus at `/museum/vltd`. The complete authorized work order is `docs/GALLERY-MAP-ROOM-EDITING-OVERNIGHT-PASS-2026-09-11.md`. Read it before editing.

EK approved this direction: enlarge the **outer** room/map workspace to the exact area marked by her blue lines (below the controls, beside the left edge, across the former right-panel width, and near the viewport bottom), keep the floor plan horizontal with the entrance on the left, and avoid all internal horizontal/vertical scrolling. The prior pass enlarged/rotated the inner SVG but did not expand the parent workspace enough; do not repeat that mistake.

Remove the left **Arrange Shelf Order** panel, the `13 rooms · 20 doors` badge, and the entire right **Floorplan / Universe Rooms** panel. Reclaim their width. Keep the map quiet: each selectable room shows only its dynamic name and `occupied / capacity items` (for example `25 / 35 items`) from the same canonical slots rendered and saved in the 3D room.

Room selection opens a compact Gallery Builder-style **Edit Room** flow for name, entering/previewing the room, item placement, existing wall finish/wallpaper controls, autosave state, and Done. In 3D Organize mode, occupied items themselves are selected/moved; empty numbered slots expose `+`; selected items expose `−`; removal affects only the displayed room and never deletes the vault item. Invalid drops do nothing. Occupied drops require a Replace/Cancel prompt and replaced items return to the vault. Desktop drag, touch press-and-hold, phone Move-to-position, and keyboard Move/Remove must all use the same slot commands and persistence.

Known baseline at brief creation: `origin/main`/production `8e2cb7c`, with the data-driven 13-room/20-door map in `MuseumCampusOverview.tsx`. Public-campus layout, navigation, targets, rooms, signs, and themes are expressly out of scope. Complete the full shared implementation, production deploy, and evidence set; do not stop after cosmetic removals or one example slot. EK's own review remains the acceptance gate.
...
- **✅ Private Photos as a paid feature — built 2026-09-19, see the dated
  entry near the top of this file for the full writeup.** Went with the
  second-private-bucket architecture (rejected the signed-URL-for-every-
  image rewrite as both riskier to ship and worse at scale). Not yet
...
- **`renderer.compile()`** — instrumented all 3 call sites with real timing; measured 90ms / 280ms (976 material refs) / 63ms, ~433ms total, once, at load. Not a multi-second-stall candidate. (A prior session's comment claiming a 40+ second stall from this call does not match current reality.)
- **Real-time lighting/shader cost** — live A/B test: `__debugSetNonHemiLightsVisible(false)` disabled all 164 non-hemisphere lights (spot/point), measured frame time 3 times in a row (baseline/no-lights/restored, 3s samples each). Result: **zero measured difference** (95ms/105ms/100ms avg, all ~10fps) — this conclusively rules out per-pixel lighting cost, since three.js excludes invisible lights from the shader's per-fragment light loop entirely.
- **Software/no GPU acceleration** — checked `UNMASKED_RENDERER_WEBGL` directly: `ANGLE (Intel, Intel(R) Iris(R) Xe Graphics ..., D3D11)`. Real hardware acceleration confirmed, not a SwiftShader/software fallback.
- **A scene-graph leak** — `getSceneStats()` is stable across fresh reloads (mesh/material/texture/light counts identical each time), so it isn't slowly growing during a session.

### Confirmed real root cause

`renderer.info` (three.js's own real counters, exposed live via a new `__debugGetRendererInfo()` debug method) showed **1,301 draw calls per frame against only ~99,454 triangles** — the triangle count is trivial for any modern GPU, but 1,301 *separate* draw calls is a heavy CPU-side submission-overhead cost on integrated graphics, independent of per-pixel shading complexity. This matches the null result from the lighting A/B test exactly (draw-call count doesn't change when lights are toggled).

A new `__debugMeshHistogram()` debug method (breaks the scene down by parent group and geometry type) found the concentration: of 581 meshes in the live campus scene, **349 are `BoxGeometry`** (60% of all meshes) — these are the walls/floors/ceilings/trim built by `campusRoomBuilder.ts`'s shared shell builders (`buildRoomShell`/`buildNeutralShell`/`buildCeilingAndTrim`) across every room in the campus, each currently its own individual mesh/draw call, never merged. (239 `PlaneGeometry` meshes are artwork/photo frames — each has a unique item image texture, genuinely can't be merged. 55 `CircleGeometry` are small ceiling-fixture discs.)

Real unclamped frame timing (measured via an independent `requestAnimationFrame` timer, NOT the app's own `frameLog`, which clamps `dt` to 50ms for movement-physics safety and was misleadingly showing "avgFrameMs: 49.2, no spikes" earlier in this investigation — that field cannot show anything worse than 50ms even when the real frame took 200ms): sustained ~10-16fps standing still with zero input, worst frames 85-170ms. This is a steady, sustained cost, not a rare spike — consistent with "runs horrible in general," and severe enough on a bad frame to explain the doorway stalls and the literal "Page Unresponsive" dialog too.

### Already shipped this session (real fix, but confirmed NOT to touch the live campus)

Commit `b467018` converted Vault/Loft's decorative corner rivets (up to 48 per Vault room, 12 per Loft room — each previously its own individual `THREE.Mesh`/draw call despite sharing identical geometry+material) into a single `THREE.InstancedMesh` per room in `galleryRoomFinishes.ts` (`addLoftArmor`/`addVaultArmorRefined`). This is real, safe, verified (tsc/eslint/build clean, zero visual change — same geometry/material/positions, just batched). **However**, live testing after deploy showed `renderer.info` draw calls completely unchanged (1301, byte-for-byte identical) — `VltdMuseumCampus.tsx` never calls `addVaultArmor()`/`addLoftArmor()` from this file at all. That code path only feeds the personal Gallery Builder / Museum Builder preview, not the shared public campus. **The fix is real and should stay, but it is not the fix for what EK is experiencing.**

While confirming this, found that the live campus actually calls a **different, separate** armor/decorative-geometry system: `addStyledRoomArmor()` in `museumRoomArmor.ts`, called unconditionally for every `EDITABLE_ROOM_IDS` room from `VltdMuseumCampus.tsx` around line 1523. **This has not been checked yet** — it likely has its own unbatched rivet/rib/divider pattern analogous to what was just fixed in `galleryRoomFinishes.ts`, and is a plausible secondary contributor to the 1,301 draw calls. Check this file first before starting the larger wall-merge work below — it may be a smaller, safer, faster win.

### The PLAN — not yet executed, scoped here so another chat can pick it up

Merge each room's static, same-material wall/floor/ceiling/trim `BoxGeometry` meshes into fewer combined meshes per room (via `THREE.BufferGeometryUtils.mergeGeometries`, grouped by which material each piece already uses — walls typically share one `wallMaterial`, ceiling trim shares one `ceilingTrimMaterial`, etc.). This targets the 349-`BoxGeometry`/60%-of-scene concentration directly. Recommended approach: **room-shell-type by room-shell-type** (`buildRoomShell` first, since it's used by the 3 fully-converted rooms POP_CULTURE/TCG/COLLECTION and is the newest/most-understood code; `buildNeutralShell`/legacy rooms after), with a live visual check (screenshot from inside the room, all 4 walls, doorways) after each type before moving to the next — **not** one giant sweeping change across every room at once, given how many hand-tuned correction passes this shared code has already been through (see the many "EK's Nth correction" comments throughout `campusRoomBuilder.ts` and `galleryRoomFinishes.ts`).

**Two concerns EK raised, both checked and confirmed safe before starting:**

1. **Does merging break wallpaper/theme changes?** No. Verified directly: theme/style swapping (`VltdMuseumCampus.tsx` ~line 1536, `shell.wall.copy(styled.wall); shell.wall.needsUpdate = true;`) already works by mutating the properties of **one shared material object per room** in place — every wall mesh in a room already references that same single material instance today, not its own separate copy. Merging same-material geometry into one mesh changes nothing about this: the merged mesh still points at that identical shared material object, and a theme swap still instantly re-textures it exactly as it does now. **The one real implementation detail to carry through the merge:** a few places currently hide/show *individual named mesh references* directly (e.g., the Grand Hall ceiling-trim-hiding logic added earlier this session, which reaches into `shellEntry(room.id).ceilingTrim`). Those call sites need to be updated to target the merged mesh instead of the old per-piece references — a mechanical update, not a design problem.
...
### Second real complaint, same session, after the fixes above: "well over 10 seconds to get into the main hall" + a stuck "Loading detail…" indicator

This is a DIFFERENT symptom from the steady-state frame rate fixed above — it's initial setup latency, not draw calls. ("Loading detail…" is not a content spinner; it's `VltdMuseumCampus.tsx`'s own stall indicator, toggled visible whenever a single frame's real delta exceeds 250ms, staying visible for 700ms after the last such frame — EK seeing it stuck means real frames were repeatedly exceeding 250ms.)

Added real timing (commits `ef3b9fe`, `e534eee`, `4c31156`, `4fd7f4d` — all temporary `console.warn` instrumentation, same pattern as the draw-call investigation) around the entire synchronous scene-construction path, from the top of the setup effect through every room-building loop to the Grand Hall GLB `.load()` call and the first `tick()`. Confirmed real: **the whole campus (all ~14-20 rooms, every wall segment, every room's textures/trim/lights) is built synchronously, blocking the main thread completely, before the page is interactive at all** — nothing progressive or deferred, no matter which room the visitor actually spawns in.

**The surprising part**: measured 4 times across separate fresh page loads in the same browser session, with zero logic changes between measurements (only debug-log additions) — **7,648ms → 4,020ms → 3,289ms → 1,167ms**, a consistent ~2x-per-run drop. This means the synchronous construction cost is NOT a fixed number — it's heavily dependent on browser/JS-engine warm-up state (V8 JIT tier-up, general engine warmth across the session), not something that stays constant run to run. A genuinely cold first visit (a new tab, first load of the session — i.e. exactly EK's situation) can plausibly land in the 7-10+ second range even on hardware where a "warm" repeat visit takes step 1 second. **Do not trust a single measurement of this number as if it were deterministic** — always measure a truly fresh, cold navigation (new tab or hard reload) if re-checking this.

**Real fix implication**: because the cost is fundamentally "build the ENTIRE campus before the visitor can do anything," the right fix is architectural, not another merge — build only the spawn room (and maybe its immediate neighbors) synchronously first, defer every other room's construction to run progressively after the first interactive frame (e.g. `requestIdleCallback`, or chunked across several animation frames). This would make first-interactive time small and CONSISTENT regardless of warm/cold engine state, instead of the current all-or-nothing synchronous block. **Not yet attempted** — this is a bigger structural change to the mount effect's shape (most of the effect currently assumes the whole campus exists synchronously before any per-room "ready" state), and deserves its own scoped pass with EK's input on which room(s) must be instant vs. can visibly pop in a moment later.

Debug hooks/logs added for this specific investigation (also temporary, same removal note as the draw-call ones): the `[perf] sync scene construction before GLB .load() call`, `[perf] TOTAL sync setup before first tick()`, `[perf] per-room buildNeutralShell breakdown`, `[perf] buildSharedWall loop`, `[perf] checkpoint after converted-room trim...`, and `[perf] checkpoint at start of Grand Hall custom block` console.warn lines in `VltdMuseumCampus.tsx`.

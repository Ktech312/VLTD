...
Ten items EK explicitly asked to defer to "the next batch of fixes" (the last one queued explicitly to run AFTER the rest), not started this session:
- 2FA modal: pressing Enter in the code input does nothing (has to be clicked) — root cause already found, not yet fixed: the input isn't inside a `<form>` and has no `onKeyDown` handler.
- Museum (`/museum/vltd`): EK reports still getting stuck in doorways when scrolling in. Not investigated. See CHECKLIST.md's entry for a starting-point hypothesis (this session's own recent jamb/wall geometry fix and the new MISC↔HUB door both touched exactly this kind of collision-adjacent geometry) — needs EK to say which door(s) before it can actually be root-caused.
- Museum (`/museum/vltd`): EK reports it's very slow/choppy when looking up or turning sideways. Not investigated — not obviously the same texture-payload lag already fixed (`a80b4f0`), since that was a load-time fix, not a runtime frame-rate one. See CHECKLIST.md's entry for where to start (real frame timing during a drag, and whether it's HUB-specific given the new 7.9MB Grand Hall ceiling GLB or campus-wide) — needs EK to say which before it can be root-caused.
- HUB Grand Hall skylight: 4 free-floating lights down the skylight's centerline read as unsupported props (EK's annotated screenshot, 2026-09-14) — either remove them or add a visible chain/rod so they read as hanging pendants. See CHECKLIST.md's entry — need to confirm whether these are the same "four warm point lights spread across the coffer ring" from the 2026-09-13 Grand Hall pass (which are supposed to stay) or a separate set.
- Museum doorway nav targets: restyle the square direction-indicator boxes at each doorway into chevrons pointing the direction that doorway faces; the room-center target should show 4 small chevrons, one per cardinal direction, instead of one square. Same size/color as today, shape only. See CHECKLIST.md's entry.
- Museum artwork: some framed pieces (EK's screenshot, TCG room corner) straddle a wall-segment seam — a dark vertical joint line shows through the frame itself instead of the frame sitting cleanly in front of one continuous wall face. Needs a `hangFrame()`/z-offset check in `VltdMuseumCampus.tsx` and a look across other rooms for the same issue. See CHECKLIST.md's entry.
- Museum artwork labels: restyle the plain white name-label rectangles to use the same gold-framed plaque style as the room-name signs above each doorway, but sized for the artwork, not door-sized. See CHECKLIST.md's entry.
- Museum artwork: 3 selectable frame styles, gated by shelf vs. no-shelf display. EK corrected an earlier misread: all 3 already exist in the app — (1) today's plain black/plastic-sleeve frame, restricted to no-shelf wall-hangs going forward; (2) a gold/tan-trimmed white-mat frame seen in-museum on a shelf, source not yet located in code (current 3D code only has the one plain style — needs live inspection); (3) the `/vault` page's own existing `FrameStyle` "gallery" look (gold border, "COLLECTION PIECE" card) — reuse as-is, do not rewrite. (2) and (3) both usable for wall-of-shelves. See CHECKLIST.md's entry.
- Museum entry corridor: give it the same marble floor/limestone wall finish built for HUB's Grand Hall (2026-09-13 pass), plus a real sky-with-clouds image overhead instead of the flat dark-blue ceiling/sky gradient. Confirmed via code: the "Corridor" HUD label is PLAZA's own fallback text (its `label` field is empty), so this IS PLAZA. See CHECKLIST.md's entry.
- DO LAST: Grand Hall outer compass floor artwork — replace the runtime canvas-drawn compass with a new approved PNG (`vltd-grand-hall-compass-inlay-v2.png`), keeping the existing center VLTD logo unchanged. Fully-specified work order, explicitly queued to run after every other item on this list. Full spec in CHECKLIST.md's entry.

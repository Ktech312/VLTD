...
Nine items EK explicitly asked to defer to "the next batch of fixes," not started this session:
...
- Museum artwork: 3 selectable frame styles, gated by shelf vs. no-shelf display — the current plain black/plastic-sleeve frame restricted to no-shelf wall-hangs only, a gold-trimmed white-mat frame (matching the actual Vault/Collection card UI look) for wall-of-shelves displays, third style not yet specified. This is a bigger feature (frame selection + new frame materials/geometry) than the rest of the list, not a one-line fix. See CHECKLIST.md's entry.
- Museum entry corridor: give it the same marble floor/limestone wall finish built for HUB's Grand Hall (2026-09-13 pass), plus a real sky-with-clouds image overhead instead of the flat dark-blue ceiling/sky gradient. Needs the corridor's actual room ID confirmed in `campusLayout.ts` first, and the Grand Hall marble/limestone code (currently HUB-only) generalized or duplicated for it. See CHECKLIST.md's entry.

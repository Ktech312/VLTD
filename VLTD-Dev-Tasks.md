...
- **Task 14 — Fix multi-device login/sign-out behavior (BUG, security-relevant) — code shipped `8a95a06`, EK's own cross-device test (point 5) still outstanding** — Ordinary sign-out was signing out every device the account was logged into, not just the one you were on (root cause: every `supabase.auth.signOut()` call in the app was unscoped, which defaults to "global"). Fixed per EK's exact spec:
  1. Ordinary sign-out now uses `supabase.auth.signOut({ scope: "local" })` — done, `src/lib/auth.ts`'s `signOut()` defaults to local.
  2. All three named call sites updated: `src/lib/auth.ts`, `src/lib/adminAuth.ts`, `src/components/MfaChallengeGate.tsx` — done. Full-repo grep confirmed these are the only 4 `supabase.auth.signOut()` call sites, none left unscoped.
  3. Account → Security's existing "Sign out all" action explicitly passes `{ scope: "global" }` now — done (this button already existed and called the same function being defaulted to local, so its one call site needed updating too, not just left alone).
  4. 2FA/admin protection itself untouched — done, only sign-out scope changed.
  5. **NOT DONE**: EK's own real desktop+phone side-by-side test. Needs her own two physical devices; cannot be run from a build or a single session. Report each of the 5 sub-checks separately once run.
- **Task 15 — `follows` table's follower-count query 503s on every page load (BUG, root cause unresolved, needs Supabase dashboard access)** — Overnight QA sweep found `getFollowerCount()`'s HEAD+`count:"exact"` query against `follows` (filtered by `followed_id`) returns 503 on literally every page load, 100% reproducible across many separate tests. Everyone's follower count has silently shown 0 regardless of the real number (the `count ?? 0` fallback swallowed the error with no visible sign anything was wrong). First attempted fix (commit `38473ca`, narrowed `select("*")` to `select("id")`, the standard fix for a known PostgREST HEAD-count quirk) did NOT work — confirmed live after that deploy, same 503, same query, only the select column changed. Ruled out via direct migration read: schema is healthy (indexes exist on both `follower_id` and `followed_id`, RLS is a plain public-read `using (true)` policy) — this is not a missing-index or permissions problem. Could not diagnose further from the client: no way to see the actual Postgres/PostgREST error behind a 503 from here (HEAD responses carry no body, and this session has no Supabase dashboard/log access). **Needs EK to check the Supabase project's own logs for this specific `follows` table HEAD/count request** to find the real underlying error before another fix attempt.
- **Task 16 — Same "raw `__preset:` avatar sentinel used as an `<img>` src" bug exists in several more files (BUG, one confirmed instance fixed `17a5df3`, others not yet checked)** — `resolveAvatarSrc()` (`src/lib/avatarResolve.ts`) is this app's one shared way to turn a profile's `avatar_url` into a real image src, since a preset-chosen avatar is stored as a sentinel string (`"__preset:guitar"`) rather than a real URL. `/account`'s own avatar preview was rendering that raw sentinel directly and is now fixed. A live re-check of `/account` after that fix still showed ONE more `__preset:guitar` 404 firing somewhere on the same page load with no visible broken image on screen (checked — nothing broken in the visible viewport, so it's likely an off-screen/background render, not chased further tonight). Grepped for the same raw-render pattern elsewhere and found it's NOT fixed in: `src/app/discover/page.tsx:557` (`collector_avatar_url`), `src/app/HomeClient.tsx:389`, `src/app/u/[username]/opengraph-image.tsx:118` (a server-rendered OG image — needs its own resolve path since it can't import the client lib as-is), `src/app/v/[profileId]/page.tsx:426,457`, `src/components/gallery/CollectorBioModal.tsx:195`, `src/components/gallery/GuestGalleryRenderer.tsx:567`, `src/components/social/CommentThread.tsx:118`. Each is a different rendering context (public profile pages, guest gallery views seen by OTHER users' visitors, comment threads, a server-side OG image generator) and deserves its own careful check rather than a blind find-and-replace across all of them at once — logging for a proper pass, not fixed blind overnight.

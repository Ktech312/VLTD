...
| **Users** | users | `<iframe src="/admin/users">` | Per-account activity/AI-usage analytics (joins `profiles` presence columns with `ai_usage_log`) and the 3D Museum beta-access toggle (`profiles.museum_beta_enabled`). Added because EK couldn't find where to grant 3D Museum beta access — it lived on `/admin/users` with no way in from this shell at all until this was wired up. |

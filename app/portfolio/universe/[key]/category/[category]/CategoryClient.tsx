...
export default function CategoryClient({ uKey, categorySlug }: { uKey: string; categorySlug: string }) {
  ...
  const pool = useMemo(() => {
    ...
    if (!rangeWindowMs) return onlyCat;
    const since = Date.now() - rangeWindowMs;
    return onlyCat.filter((i) => getCreatedAtMs(i as any) >= since);
  }, [items, universe, category, rangeWindowMs]);
  ...
  const bySubcategory = useMemo(() => {
    const map = new Map<string, { sub: string; count: number; value: number; gain: number }>();
    for (const i of filtered as any[]) {
      const sub = safeDecode(i.subcategoryLabel ?? "—");
      const v = clamp(Number(i.currentValue ?? 0));
      const c = clamp(Number(i.purchasePrice ?? 0));
      const g = v - c;

      const row = map.get(sub) ?? { sub, count: 0, value: 0, gain: 0 };
      row.count += 1;
      row.value += v;
      row.gain += g;
      map.set(sub, row);
    ...
    <main className="min-h-screen bg-[color:var(--bg)] text-[color:var(--fg)]">
      <div className="mx-auto max-w-6xl px-5 py-10">
        <div className="flex items-start justify-between gap-4">
          <div>
            <div className="text-xs tracking-widest text-[color:var(--muted2)]">PORTFOLIO • CATEGORY</div>
            <h1 className="mt-2 text-4xl font-semibold">{category}</h1>
            <p className="mt-2 text-sm text-[color:var(--muted)]">
              Universe: {UNIVERSE_LABEL[universe]} • Subcategories • Top 5 • Table
            </p>
          </div>
        ...
        <div className="mt-6 rounded-3xl bg-[color:var(--surface)] p-5 ring-1 ring-[color:var(--border)] shadow-[var(--shadow-soft)]">
          ...
          <div className="mt-4 flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
            <div className="text-xs text-[color:var(--muted2)]">Search filters Top 5 + subcategories + table.</div>
            <input
              value={q}
              onChange={(e) => setQ(e.target.value)}
              placeholder="Search title, grade, notes, subcategory…"
        ...
        <div className="mt-6 grid gap-3 md:grid-cols-3">
          ...
          <Card title="TOP SUBCATEGORY">
            <div className="text-lg font-semibold">{bySubcategory[0]?.sub ?? "—"}</div>
            <div className="mt-1 text-sm text-[color:var(--muted)]">
              {rankMode === "value" ? fmtMoney(bySubcategory[0]?.value ?? 0) : fmtMoney(bySubcategory[0]?.gain ?? 0)} •{" "}
              {bySubcategory[0]?.count ?? 0} items
            </div>
          </Card>
        ...
        {/* Subcategory breakdown (click to drill) */}
        <div className="mt-6 rounded-3xl bg-[color:var(--surface)] p-6 ring-1 ring-[color:var(--border)] shadow-[var(--shadow-soft)]">
          <div className="text-xs tracking-widest text-[color:var(--muted2)]">SUBCATEGORIES</div>
          <div className="mt-2 text-xl font-semibold">By subcategory (click to drill)</div>
          ...
          <div className="mt-4 grid gap-2">
            ...
            {bySubcategory.length === 0 ? <div className="text-sm text-[color:var(--muted)]">No data.</div> : null}
          </div>
        </div>
        ...
        <div className="mt-6 rounded-3xl bg-[color:var(--surface)] p-6 ring-1 ring-[color:var(--border)] shadow-[var(--shadow-soft)]">
          ...
          <div className="mt-2 text-xl font-semibold">Top 5 items in this category</div>

...
- [x] `ThemeContext.tsx`: `iconStyle`/`setIconStyle`, persisted to `localStorage` (`vltd_icon_style`), same hydration-guard pattern as the background theme. Confirmed independent of it in both directions, live on production.
- [x] `ThemePicker.tsx`: new "Icon style" section (Classic / Friendly Glass) in the existing appearance menu (TopNav + Account). Classic is default.
- [x] 109/109 `AppIconName` entries verified 1:1 against `public/ui/icon-themes/friendly-glass-v1/*.png` (scripted check).
- [x] Size floor `FRIENDLY_GLASS_MIN_SIZE = 18` — measured, not guessed, via a disposable pixel-accurate size-test harness (native vs. 4x magnification, 8-48px). Below 18px an icon always renders Classic regardless of the global style; one global constant, no per-page exceptions.
- [x] **Bug found + fixed in this session's own verification**: `filled` (saved/liked toggle) had zero Friendly Glass treatment. Added a gold ring + checkmark badge on the tile when `filled=true` and Friendly Glass is active, through the shared component (FavoriteButton, wishlist hearts, Messages star toggle all get it automatically). Classic untouched.
- [x] **Real regression found + fixed in this session's own verification**: any icon below the 18px floor rendered as nothing when Friendly Glass was active — the classic layer's own toggle class was being hidden by the global CSS rule with no friendly image to replace it. Fixed by only applying the toggle class when the friendly layer actually renders.
- [x] Live-verified on `vltd.vercel.app` (real authenticated session): TopNav, Vault (grid + item detail), Exhibitions, Museum Builder, Messages (small below-floor star toggle + Classic fill), Account Settings, and a real action modal (Social Export). Classic → Friendly Glass → Classic via the real Theme Picker UI, no layout shift, survives a hard reload, Background/Icon-style independence confirmed both directions, zero image 404s.
- [ ] **Not verified — mobile-width authenticated pass.** Claude-in-Chrome's `resize_window` reported success but `window.innerWidth` stayed at 1920 the whole session on this machine (tool limitation). Mobile TopNav/BottomNav at 375×812 was confirmed correct in an earlier pass via the sandboxed preview pane on an unauthenticated build (same code path/sizes) — doesn't cover the new filled-badge feature specifically at mobile width.
- [ ] **Not checked — a live favorited-star instance in a public gallery** (FavoriteButton via `GalleryLayout`/`GuestGalleryRenderer`). The filled-badge logic was instead verified across every size/variant combination via a disposable dev route, not hunted down on a real public exhibition.
- [x] `tsc --noEmit` clean / targeted ESLint 0 errors (3 pre-existing `no-img-element` warnings, deliberate — `next/image` isn't compatible with the dual-layer CSS-toggle approach) / `npm run build` clean on every route, both commits. Vercel production `Ready`/`success` confirmed via GitHub commit-status API for both.
...
All explicitly deferred by EK ("add that to the fix list"). Status as of this session's continuous work-through-the-list pass: 9 of 10 items fully fixed and deployed; the 10th (frame styles) is 2/3 done, the last style still unlocated.
...
- [x] **Museum artwork: gold-gradient "gallery" frame style added, selectable per room** — 2 of 3 styles fixed, `7300de0`. "classic" (today's look, no-shelf only per EK) and "gallery" (gold-gradient bezel, confirmed live as the Exhibitions builder's own `PremiumDisplayCard`) are real, threaded through `hangArtPreservingAspect`/`placeItemsAtSlots`/`placeArtwork`, selectable in Museum Builder next to the existing room-style/shelf-capacity controls, persisted via new `setRoomFrameStyle()`/`museum_room_meta.frame_style`. **Migration not yet run by EK** — see the SQL block reported to her separately; the app degrades safely (falls back to "classic") until she runs it. Shelf slots always render "gallery" regardless of the room setting, enforcing the no-shelf-only rule automatically. The 3rd style (gold/tan-trimmed white-mat frame seen in-museum on a shelf) is still unlocated in code after a live-inspection pass — flagged, not guessed at, not built.
- [x] **DO LAST — Grand Hall outer compass floor artwork** — fixed, `84d4102`, done last as instructed. Copied the approved PNG from EK's main checkout (SHA-256 verified against her expected hash), replaced the runtime canvas-drawn compass with it at the same footprint/position, kept the center VLTD logo mesh untouched. Confirmed live: renders instantly (no shader-compile stall, confirming the choppy-turning fix above also covers it), sits flush with the marble floor.

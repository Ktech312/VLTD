...
## 2026-09-14 — 2FA lockout: EK reported it, root-caused, fixed, confirmed back in
Unrelated to the ceiling/museum work — EK's own root-cause report traced it to the global 2FA gate (`MfaChallengeGate.tsx` + `Providers.tsx`, added 2026-09-12), not anything ceiling-related. Full narrative in HANDOFF.md; quick-scan status:
- [x] `91d0d7a`: `VltdMuseumAdminGate.tsx`/`MuseumBuilderOwnerGate.tsx`/museum Builder-button check all used to call `getMyAdminRole()` once on mount and never recheck, showing a hard "Not authorized" for BOTH a real non-admin and an admin whose session just hadn't finished 2FA yet — and never recovering even after a successful verify. Now use `getMyAdminAccessStatus()`, show a neutral "finishing 2FA" message instead, and recheck on every auth-state change.
- [x] Same commit also added a 400ms settle-and-recheck to `MfaChallengeGate.tsx`'s `checkAal()`, meant to stop the modal reopening on ordinary navigation of an already-verified session.
- [x] `78cad6a`: EK reported that made things WORSE — couldn't reach the 2FA modal at all. Reverted the 400ms delay (real race: overlapping `onAuthStateChange` events + no request sequencing = a stale call could clobber a fresh one). Root-caused a real, PRE-EXISTING, unrelated bug while investigating: `globals.css`'s `body > * { position: relative; z-index: 1; }` (for noise-layer stacking) unconditionally overrides the modal's `.fixed`/`z-[999999]` classes, because Tailwind v4's utilities live in `@layer utilities` and unlayered CSS always wins over layered CSS regardless of specificity. The modal has likely never rendered as a real overlay since it was built. Added a scoped `body > .fixed { position: fixed !important; }` override.
- [x] `df10d33`: position was fixed but the input still wasn't clickable — same rule ALSO stomps `z-index` (the position-only fix didn't touch that). Added a second scoped override, `body > .z-\[999999\] { z-index: 999999 !important; }`.
- [x] **EK confirmed live: back in, modal clickable, verify works.**
- [ ] **Deferred to next batch, EK's own request**: pressing Enter in the 6-digit code input does nothing — has to be clicked. Root cause already found, not yet fixed: the input isn't inside a `<form>` and has no `onKeyDown` handler, so there's nothing for Enter to trigger. Fix is small — wrap in a `<form onSubmit>` or add an Enter-key handler to the input, calling the existing `verify()`.

## Next batch of fixes — EK's punch list, reported 2026-09-14
All explicitly deferred by EK ("add that to the fix list"). Status as of this session's continuous pass:
- [x] **2FA modal: Enter key doesn't submit** — fixed, `a99c321`. Wrapped the code input in a `<form onSubmit>` calling the existing `verify()`. tsc/eslint/build clean, deployed, confirmed live.
- [x] **Museum (`/museum/vltd`): stuck in doorways when scrolling in** — fixed, `d1815e0`. Root-caused without needing EK to name a specific door: `moveWithCollision()` only ever tried the full combined (x,z) step and stopped outright the instant that point left walkable ground; a doorway's own walkable "bridge" is exactly the door's visual width with zero side margin (unlike a room's own floor, inset a full `WALKABLE_MARGIN` from every wall), and `onWheel`'s scroll-forward delta follows the camera's raw facing direction, not the doorway's axis — so scrolling in even slightly off-center clips the bridge edge and halts completely. Now slides along whichever single axis is still walkable (standard fix for this collision class). Deployed; not yet re-tested live by EK.
- [x] **Museum (`/museum/vltd`): very slow/choppy when looking up or turning sideways** — fixed, `d1815e0`. Measured live via `getWheelDiagnostics()`/`getSceneStats()`: looking up at the Grand Hall ceiling GLB the first time after page load stalled 40+ seconds; a repeat measurement immediately after took 53ms for 5 renders — confirmed a one-time shader-compile stall (not per-frame cost; only 7 of the scene's 167 lights are ever active at once, ruling out lighting load). `populateDynamicContent()` already precompiles the rest of the scene's materials for exactly this reason, but this GLB loads independently on its own schedule and was never included in that pass — now precompiled the moment its ~300 materials are added. Deployed; not yet re-tested live by EK. Note: this covers the single largest source of never-before-seen materials (the GLB) but isn't exhaustive — an item-art texture that finishes loading later still compiles on its own first appearance, same disclosed limitation the existing precompile call already carries.
- [x] **HUB Grand Hall skylight: remove the 4 free-floating ceiling lights** — fixed, `d1815e0`. Confirmed live via `debugMeshesInRegion()`: 6 (not 4 — EK's screenshot only framed part of the ceiling) plain `CircleGeometry` discs baked into the GLB along the skylight centerline, separate from and unrelated to the coffer-ring wall-wash lights (which are untouched). Confirmed these aren't real `THREE.Light` objects, so hiding them changes nothing about the Hall's actual illumination. Removed per EK's own first option (no chain added — no rafter/purlin mesh in this asset to anchor one to without guessing at ungenerated geometry).
- [x] **Museum artwork: frames straddling a wall-segment seam** — fixed, `77eff75`. Root-caused via live measurement, not guessing: the standard wall-art stand-off (0.19 from the wall's own centerline) was shallower than the Vault-armor divider/rivet strips `museumRoomArmor.ts` mounts on that same wall (measured live at 0.215 from centerline) — wherever a frame landed near a divider's x position, the divider's own front face sat physically in front of the frame. Bumped the shared campus-wide margin to clear both Vault and the deeper Loft-style armor.
- [x] **Museum artwork labels: restyle to match the door-sign plaque** — fixed, `77eff75`. `hangCompactLabel()` now reuses `buildDestinationSign()`'s own gold-framed plaque construction (same cached face texture + label renderer), sized for the item (0.9–1.7 wide) instead of the door sign's fixed size — not tagged as a destination sign, so it's never swept by the room-retitle traversal.
- [x] **Museum entry corridor (PLAZA): marble floor/wall + sky with clouds** — fixed, `d1815e0`. Confirmed "Corridor" HUD label is PLAZA's own empty-`label` fallback text. Added the same thin-overlay marble-floor/limestone-wall technique as the Grand Hall pass (PLAZA is `noWalls`, so its visible walls are actually SPOTLIGHT's/STORE's own — this adds a thin skin flush against the inward side of each without touching their own finish). Sky background widened from an 8px gradient strip to 512x256 with real cloud shapes drawn in (procedural, matching this codebase's existing canvas-texture convention — no external/stock photo used).
- [x] **Museum doorway nav targets: chevrons** — fixed, `fa51153`, confirmed live. Single chevron per doorway target rotated to that waypoint's own real `destinationYaw`; 4 small chevrons (one per cardinal direction) for room-center targets. Verified correct orientation live via screenshot at the PLAZA↔HUB entrance.
- [ ] **Museum artwork: 3 selectable frame styles, gated by shelf vs. no-shelf display.** EK's 3 screenshots (2026-09-14) plus her own correction after an initial misread: all 3 styles already exist somewhere in the app today — (1) the current plain black/plastic-sleeve frame (`hangFrame()`/`hangArtPreservingAspect()` in `VltdMuseumCampus.tsx`/`campusRoomBuilder.ts`, confirmed identical for both wall and shelf placement today) — EK wants this restricted to NO-shelf wall-hangs only going forward; (2) a gold/tan-trimmed white-mat frame shown in-museum on a shelf — NOT found yet in any 3D museum code path searched so far (`hangFrame`, `hangArtPreservingAspect`, `museumRoomFurniture.ts` all currently only build the one plain black style) — needs locating, possibly in Museum Builder's own GLB-based rendering or a room-style-specific branch not yet found, live-inspection likely required; (3) the Vault/Collection card look ("COLLECTION PIECE", gold border) — EK confirmed this is `/vault` page's own existing `FrameStyle` system (`getFrameClasses()` in `VaultInner.tsx`, values `gallery`/`shadowbox`/`slab`/`minimal`/`vault` — the "gallery" case's colors, `#d9b277`/`#7f633f`/`#f4ead8`, are the closest match by inspection) — EK says reuse it exactly, do not rewrite it, just bring that same visual into the 3D museum. Both (2) and (3) should be selectable for wall-of-shelves displays. Next step before implementing: live-inspect the actual `/museum/vltd` and `/vault` pages to positively confirm exactly which code renders each of EK's 3 reference images, since (2)'s source is still unconfirmed by code search alone.
- [ ] **Museum entry corridor: match Grand Hall's marble floor/wall finish, and add a sky-with-clouds image overhead.** EK's screenshot (2026-09-14, entry corridor leading to the "VLTD MUSEUM" door/PLAZA) shows the corridor still on the plain gray shared-shell floor/wall finish with a flat dark-blue ceiling — EK wants the same marble floor + limestone wall finish built for HUB's Grand Hall (2026-09-13 custom design pass: warm-ivory-marble field + charcoal-marble border floor, ivory-limestone walls) applied here too, plus a sky/clouds image visible overhead (the existing flat dark-blue gradient ceiling/sky, from the 2026-09-10 PLAZA open-air sky-gradient `CanvasTexture` work, should become an actual sky-with-clouds image rather than a plain gradient). Needs to identify which room this corridor actually is in `campusLayout.ts` (looks like PLAZA or an approach corridor to it) before reusing the Grand Hall marble/limestone code — that code was built HUB-only inside `VltdMuseumCampus.tsx`'s `roomWallMaterial()`/floor-overlay branch and would need generalizing or a second room-specific branch, not a copy-paste.
- [ ] **Museum doorway nav targets: restyle from squares to chevrons.** EK asked (2026-09-14) to change the room-direction indicator boxes at each doorway into chevron/arrow shapes that point in the direction the camera will face after using them — same size and color as today, just the shape. The one at the center of a room (the room's own waypoint target, currently `HUB_TARGET_SIZE`/2.2-unit squares built in the waypoint-mesh loop in `VltdMuseumCampus.tsx`) should instead show 4 small chevrons, one pointing each of the 4 cardinal directions, rather than one shape. Needs a plan for how direction is derived per-target (likely from each doorway's own wall axis / the room-to-room vector already used for `startWalkTween`'s destination yaw) before implementing.

- [ ] **DO LAST — Grand Hall: replace the outer compass floor artwork with a new approved PNG asset.** Fully-specified work order from EK (2026-09-14), explicitly queued to be done AFTER every other item above. Add `public/museum/grand-hall/vltd-grand-hall-compass-inlay-v2.png` and use it as the outer floor-compass texture in `VltdMuseumCampus.tsx`, replacing the runtime `medallionCanvas`/`CanvasTexture`-drawn compass rings (~line 843-879 as of this session). Keep: the outer compass's current ~9-unit-radius footprint/placement centered in the Hall; the transparent center; the existing `/brand/vltd-museum-floor-medallion-v1.webp` logo centered inside, unchanged; the new PNG's own proportions (no stretch/crop/recolor/redraw, no cyan lighting added). Technical requirements: sRGB color space + `renderer.capabilities.getMaxAnisotropy()` (same pattern as every other floor/wall texture in this pass); alpha/transparency handled so the open center and outside corners don't render as black squares; keep the compass and the center logo at slightly different Y so they don't z-fight, while both staying visually flush with the marble floor. Do not touch the marble floor, ceiling, Hall dimensions, lighting, doors, nav targets, camera, movement, or any other room. Remove the old canvas-drawing code only after confirming nothing else references it (`vltdSeal`/the center-logo mesh is separate and stays). Verify from the Hall center and several doorways: new compass surrounds the logo correctly, logo stays sharp/centered/fully visible, no black squares, no stretch/clip/overlap/flicker, flush with the floor, movement/load performance unchanged. Report commit, push, deployment status, and exactly what was visually verified — scope is the outer compass artwork only.
...
Work order: `docs/GRAND-HALL-CUSTOM-DESIGN-2026-09-13.md` + `docs/GRAND-HALL-VISUAL-ASSETS.md` (both authoritative, both read in full before writing any code, along with both reference images). Replaces the previous placeholder "Grand Hall enhancement" block (a flat emissive plane skylight + a canvas-drawn medallion) in `VltdMuseumCampus.tsx` with real geometry, on the live production `/museum/vltd` HUB room — not Museum Builder, not a preview.
- [x] **Floor**: warm-ivory-marble field + charcoal-marble perimeter border, real PBR textures (basecolor/normal/roughness, `THREE.TextureLoader` + `colorSpace` + `renderer.capabilities.getMaxAnisotropy()` — the exact same loading pattern the existing VLTD floor-seal texture already used), layered as a thin overlay on top of HUB's existing shared-shell floor (same inlay technique the seal itself already uses against the compass medallion beneath it) — no changes to `campusRoomBuilder.ts`'s shared floor-building code.
- [x] **Walls**: real ivory-limestone PBR texture replaces `HUB_FINISH`'s flat gold wall color — applied via one HUB-only branch inside `VltdMuseumCampus.tsx`'s own `roomWallMaterial()`, not by touching `createWallMaterial()` or `HUB_FINISH` — every other room's wall material is completely unaffected.
- [x] **Wall base trim**: charcoal-marble overlay on HUB's existing baseboard, built from the same wall-segment/door-gap walk `buildRoomTrim()` already does internally (`computeCampusWallSegments()`/`splitSegmentForDoor()`), reproduced locally in `VltdMuseumCampus.tsx` so no shared function needed changing.
- [x] **Ceiling**: HUB's existing flat shared-shell ceiling plane is left in place but set invisible (found by material identity via `roomShellMaterialsByRoomId`), replaced from below by a real coffered ceiling — a symmetrical 3x3 grid (8 broad coffer bays around a long central skylight, all sized as fractions of HUB's own real `w`/`d`/center so the layout can never drift out of sync with the room's real doorway axes) — each coffer a recessed warm-ivory-plaster panel with a warm emissive reveal strip around its edge and one small centered downlight disc.
- [x] **Skylight**: a dark-bronze framed curb at the ceiling plane, then a real deep well (side walls + glass top, not a flat plane) with a dark bronze mullion grid on the glass that continues down the well's own side faces.
- [x] **Lighting stays bounded**: one cool point light for the skylight's daylight, four warm point lights spread across the coffer ring — no per-coffer real light (the coffer glow itself is emissive-material-only, per the design doc's explicit "limited number of lights" instruction).
- [x] VLTD medallion/seal mechanism (compass-rose canvas + the real `vltd-museum-floor-medallion-v1.png` inlay) is byte-for-byte unchanged, just re-sequenced to run after the new marble floor so it sits correctly on top of it.
- [x] Confirmed via diff: `campusRoomBuilder.ts` has ZERO changes (not in the diff at all) — every "do not touch" item (HUB's real dimensions/walls/doors/casings/transoms/signs/nav targets, camera, movement, every other room, Museum Builder, Gallery Builder, `organizeSlots.tsx`, `MuseumRoomPopup.tsx`, `MuseumBuilder.tsx`, `museumRoomArmor.ts`, `museumRoomFurniture.ts`) is untouched. `git diff --stat`: exactly 1 file changed (`VltdMuseumCampus.tsx`).
- [x] `npx tsc --noEmit` clean (0 errors) / targeted ESLint on the one changed file, 0 errors (1 pre-existing, unrelated rule-suppression warning carried over) / `npm run build` clean, exit 0 — ran a real `npm install` first (this worktree had no installed dependencies).
- [ ] **Not visually verified — no browser/render capability in this session.** The parent session and EK do the required live check (main entrance, every interior doorway, all four corners, the room center, straight up) before this is accepted as finished.
...
Work order: `docs/POP-CULTURE-VAULT-PARITY-2026-09-13.md` (disclosed as this session's own interpretation of an overnight instruction referencing prior "source-checked" instructions that could not be located anywhere in this session's transcript — EK may redirect in the morning). Closes the "Noted, not done" follow-up left at the bottom of the 2026-09-12 Vault-style full-parity entry directly below.
- [x] **DB check confirmed, not assumed**: `20260912_museum_room_style.sql` has been run (`museum_room_meta.room_style` column exists); POP_CULTURE's own row already has `room_style = "vault"` saved. Checked via a read-only script (service-role key from `.env.local`, same pattern as the earlier MFA-fix session) — deleted after use.
- [x] `museumRoomArmor.ts`'s `addStyledRoomArmor()` — already built/proven in Museum Builder (2026-09-12) — is now ALSO called from `VltdMuseumCampus.tsx`'s existing style-patch loop, per `EDITABLE_ROOM_IDS` room, using each room's own real wall segments. Unconditional call per room; the function's own style dispatcher no-ops for anything but vault/loft, so an unstyled room renders exactly as before.
- [x] `museumRoomFurniture.ts`'s `buildDisplayCase()`/`buildShelfBoard()`/`createShelfMaterial()`/`placeItemsInCases()` — also Museum-Builder-only until now — wired into the same loop, sized from each room's own real `museum_room_meta.case_capacity`/`shelf_capacity` (via `campusRoomBuilder.ts`'s existing `computeRoomCaseSlots`/`computeRoomShelfSlots`/`computeRoomShelfSpans`), independent of `room_style`. `null`/`0` capacity still means "off" (unchanged, matches that column's own documented meaning); ONE additive default — a room with a saved style but a genuinely never-set (`null`/`undefined`, not a stored `0`) `case_capacity` defaults to 5 (the personal Vault's own real `CABINET_SPOTS` count) — gated on the room having a style, so an unstyled room with no capacity row (TCG/COLLECTION today) stays at zero cases.
- [x] Curated-item placement (`buildSlotAssignments`) now includes each room's case/shelf slots alongside its wall slots, wall slots first in the same order as before — an existing room's wall-only assignment is unaffected; case/shelf slots only ever catch items that would otherwise go unplaced.
- [x] **Item interaction, real gap found and closed**: the live campus has no click/hover item-info system at all (its raycasting only targets floor waypoints) — the real existing "info treatment" is `hangCompactLabel`'s always-visible title plaque under wall/shelf items. Case items never got this, even in Museum Builder. `hangCompactLabel` exported from `campusRoomBuilder.ts`; `placeItemsInCases()` gained an optional `placeLabel` callback (Museum Builder's own call site passes nothing, unaffected) now used by the live campus to mount a title plaque on each case's front face. `computeRoomCaseSlots()`'s case-slot `rotationY` (previously a hardcoded, never-consumed `0`) now carries the real per-wall outward rotation for this — confirmed dead data before this pass, zero effect on any existing caller.
- [x] Verified the mechanism generalizes (build-order step 3): a scratch script (deleted after use) called the new functions directly against POP_CULTURE, TCG, COLLECTION, SPORTS, and CARDS's real geometry — case/shelf slots land in-bounds with no overlaps for every one of them, armor builds without throwing (86-92 meshes each), confirming this is not a POP_CULTURE-only fix.
- [ ] **Wall-armor visual result — still genuinely open.** Re-confirmed the wall-face-offset math from fix commit `2e20446` is internally consistent (`buildSharedWall()` really does center each wall box on `segment.fixed ± wallThickness/2`), but the work order disclosed that a live check in Museum Builder STILL showed no visible panel dividers/rivets even after that fix, and no further fix landed after it. **This session has no browser/visual verification capability** — could not check, in Museum Builder or the live campus, whether armor is actually visible/well-lit once rendered. Not claiming this is fixed; flagging it exactly as open as the work order left it.
- [x] Confirmed via diff: `campusLayout.ts`, `MuseumBuilder.tsx`, `VirtualGalleryRoom.tsx`, `galleryRoomFinishes.ts` all byte-for-byte unchanged (only their exports reused) — room geometry/doors/doorways/nav waypoints/camera/movement/collision/map untouched. HUB/Grand Hall block in `VltdMuseumCampus.tsx` untouched — rebased onto the concurrent Grand Hall agent's landed changes before pushing, kept its HUB-scoped lines as-is.
- [x] `npx tsc --noEmit` clean (0 errors) / targeted ESLint on all 3 changed files, 0 errors (1 pre-existing unrelated rule-suppression warning carried over) / `npm run build` clean, exit 0 — ran a real `npm install` first (this worktree had no installed dependencies).
- [ ] **Not live-verified — cannot be, no browser available this session.** A clean build is not evidence of a correct visual result. The parent session/EK need to load `/museum/vltd`, stand in POP_CULTURE, and compare it against the real personal Vault room before this counts as done or "matching the Vault."
...
Work order: `docs/MUSEUM-VAULT-STYLE-FULL-PARITY-2026-09-12.md`. Direct follow-up after EK compared the shipped Vault style (commit `bf92a5d`) against the real Vault room in the personal Gallery Builder and said it "does not look anything like" it.
- [x] `museumRoomFurniture.ts`'s `buildDisplayCase()`/`createShelfMaterial()` now take the room's own `createStyledRoomFinishes()` instance and reuse its real `dark`/`glass`/`brass` materials (same instances already used for that room's walls/rail) when a style is set; today's hardcoded colors stay the fallback for an unstyled room. `createGalleryFinishes()` gained exactly one additive line (`glass` added to its return object, same narrowly-additive pattern as the prior pass's ceiling/charcoal addition) — nothing else in `galleryRoomFinishes.ts` touched.
- [x] New `src/lib/museumRoomArmor.ts` adapts (not copies) `addVaultArmor()`/`addLoftArmor()`'s real rib/rivet/divider/seam/ceiling-lattice geometry-generation logic — same technique and materials, parameterized by each real museum room's own solid wall spans (via `campusLayout.ts`'s `roomBounds()`/`splitSegmentForDoor()`, the same data `buildSharedWall()` already uses) instead of the personal room's one fixed size. Wired into `MuseumBuilder.tsx` behind the same `roomStyle === "vault" || roomStyle === "loft"` gate `VirtualGalleryRoom.tsx` itself uses.
- [x] Two disclosed simplifications: Loft's archway-flanking jamb boxes dropped (museum doorways already get real casing from `buildSharedWall()`; avoids reproducing a known z-fighting bug); Loft's `LineSegments` "bay outline" dropped in favor of the kept divider boxes (avoids a scene-teardown leak — `MuseumBuilder.tsx`'s cleanup only disposes `THREE.Mesh`).
- [ ] **Noted, not done**: `VltdMuseumCampus.tsx` (the live public campus) has the identical armor gap for any editable room with a saved Vault/Loft style — out of scope for this pass (EK's comparison and the work order both named Museum Builder specifically), flagged as a follow-up.
- [x] Confirmed via diff: real campus geometry/doors/floor targets/navigation targets/camera/movement/collision/`CAMPUS_ROOMS`/`CAMPUS_DOORS` untouched (`campusLayout.ts` not in the diff); Gallery Builder's own files (`VirtualGalleryRoom.tsx`, `galleryTextures.ts`) untouched, read-only; Museum Builder's Organize system/item placement/save logic/Room-Map toggle/per-item Values untouched. `git diff --stat`: 3 files changed + 1 new file.
- [x] `npx tsc --noEmit` clean (0 errors) / targeted ESLint on all 4 changed/new files, 0 errors (1 pre-existing, unrelated rule-suppression warning carried over) / `npm run build` clean, exit 0 — ran a real `npm ci` first (this worktree had no installed dependencies).
- [ ] **Not live-verified.** The parent session compares this directly against the real Vault room in a live browser before calling it done.
...
Work order: `docs/MUSEUM-BUILDER-REAL-GALLERY-ENVIRONMENTS-2026-09-12.md`. EK, verbatim, after testing the pass below: "These circles, I DO NOT WANT, I NEVER ASKED YOU TO MAKE THEM, REMOVE THEM AND GIVE ME THE ONES IN THE 3D GALLERY!!!!! LITERALLY ASKED SEVERAL TIMES." Supersedes the invented `ROOM_BACKGROUND_OPTIONS` system from the entry directly below — removed entirely, not kept alongside the real thing.
- [x] `ROOM_BACKGROUND_OPTIONS`/`backgroundWallColorHex`/`setRoomBackground` removed entirely (`campusRoomBuilder.ts`/`museumCampusConfig.ts`); no code reads or writes `museum_room_meta.background_id` anymore. The BACKGROUND swatch picker UI removed from `MuseumBuilder.tsx`. The `background_id` column itself is left alone in the database (dropping it is unnecessary churn).
- [x] `galleryRoomFinishes.ts`: exactly one additive line — `createGalleryFinishes()`'s return statement now also returns `ceiling`/`charcoal` (already built internally, not previously returned). Nothing else in the file touched; `VirtualGalleryRoom.tsx`'s own use of it unaffected, that file byte-for-byte unchanged.
- [x] New Style control in `MuseumBuilder.tsx`: White/Vault/Arcade/Industrial Loft, matching `VirtualGalleryRoom.tsx`'s own style `<select>` labels/values exactly. Persisted via new `museum_room_meta.room_style` column (migration `20260912_museum_room_style.sql`, written, **not run**, SQL pasted in the session's final report). Fails soft like every other optional column in `museumCampusConfig.ts`. Blue is NOT offered (no `createGalleryFinishes()` entry) — disclosed known gap.
- [x] `campusRoomBuilder.ts`: `buildRoomShell`/`buildNeutralShell`/`buildCeilingAndTrim`/`buildRoomTrim` gained an optional trailing `styled` (`createGalleryFinishes(style)`) parameter — when present, a room's own real wall/floor/ceiling meshes get that style's real materials, baseboard/rail get `charcoal`/`brass`, and the light rig comes from that style's own `addLighting()` (via a room-centered anchor group) instead of the generic rig. `dark` is returned/available but not wired to any museum mesh this pass — disclosed gap, not an oversight. Room geometry/shape/door positions completely untouched.
- [x] `VltdMuseumCampus.tsx`: every editable room still builds its default finish synchronously first (no added network wait before first paint); once `room_style` loads async, that room's own already-built wall/floor/ceiling/baseboard/rail materials are patched in place (`.copy()`) and its generic light rig is swapped for the style's real one — same "build now, patch once loaded" pattern already used for titles/the old background tint.
- [x] `MuseumBuilder.tsx`: resolves the saved style before rebuilding its single-room preview scene, so it builds directly with the right materials/lighting from the start — no patch-after-load needed there.
- [x] Confirmed via diff: `VirtualGalleryRoom.tsx`, `galleryTextures.ts`, `campusLayout.ts` (`CAMPUS_ROOMS`/`CAMPUS_DOORS`), `MuseumRoomPopup.tsx`, `museumRoomFurniture.ts`, `galleryRoomSlots.ts`, `visitorController.ts`, `organizeSlots.tsx` all byte-for-byte unchanged. Museum Builder's Organize system/item placement/save logic/Room-Map toggle/per-item Values untouched.
- [x] `npx tsc --noEmit` clean (0 errors) / targeted ESLint on all 5 changed files, 0 errors (2 pre-existing, unrelated rule-suppression warnings carried over) / `npm run build` clean.
- [ ] **Not live-verified.** The parent session checks this live in a real browser immediately after deploy — not declared accepted or ready for EK here.

## 2026-09-12 (material quality parity pass) — shared museum rooms (`campusRoomBuilder.ts`) brought up to the personal Gallery Builder's own material/lighting quality bar
Work order: `docs/MUSEUM-MATERIAL-QUALITY-PARITY-2026-09-12.md`. EK's complaint: the shared museum's rooms (`/museum/vltd` + Museum Builder, both built by `campusRoomBuilder.ts`) read flatter than the personal Gallery Builder's own rooms she spent many correction passes getting right. MATERIAL/LIGHTING QUALITY ONLY — no campus geometry/doors/floor targets/navigation/camera/movement/collision touched, no generic fixed-box room shape swapped in; every room keeps its own real shape.
- [x] **Wall material tuning per RoomFinish.** `RoomFinish` gained `wallRoughness`/`wallMetalness` (matching Gallery's own `FinishPalette` shape); `createWallMaterial()` now reads them instead of a flat 0.85/0 for every room. `NEUTRAL_PREVIEW_FINISH` (converted rooms' base finish) reuses Gallery's own whitebox values verbatim (0.94/0 — its wallColor/railColor are the exact same hex as Gallery's whitebox palette); `NEUTRAL_LEGACY_FINISH` and `HUB_FINISH` get their own tuned values in the same range Gallery's palettes use.
- [x] **Contact shadows under wall-hung and shelf items.** `museumRoomFurniture.ts`'s existing display-case shadow generator (`caseShadowTexture`, a soft radial-gradient canvas) is now exported and reused (not reimplemented) by `campusRoomBuilder.ts`'s `hangArtPreservingAspect()` — a new `slotKind: "wall" | "shelf"` parameter draws a horizontal grounding shadow under shelf items (same technique as a display case's floor shadow) or a wall-parallel shadow behind wall-hung frames. `placeItemsAtSlots()` passes `slot.kind` through; the older `placeArtwork()` path defaults to "wall" (unchanged behavior).
- [x] **Trim/rail tuning per RoomFinish.** `RoomFinish` gained `trimMetalness`/`trimRoughness` (same `FinishPalette` shape); `buildRoomTrim()`'s `railMaterial` now reads them instead of one flat 0.5/0.35 pair for every finish.
- [x] **Real lighting for every legacy room.** New `buildLegacyRoomLightRig()` in `campusRoomBuilder.ts` reuses `buildRoomShell()`'s exact fixture/SpotLight/PointLight recipe (colors, intensities, cone angles, decay) — `buildRoomShell()` itself and its 3 converted-room callers are untouched — with fixture count adapted to room size (same `CEILING_BAY_SIZE` rhythm the ceiling texture already tiles at, capped at 6 rows) and wash-light distance scaled to the room's own footprint. `buildNeutralShell()` now calls it (after its existing ceiling, still skipped for PLAZA's open-air `includeCeiling=false` path) into an optional new `lights` param. `VltdMuseumCampus.tsx` wires each legacy room's rig into the SAME two-tier occupancy-based full/preview activation system every converted room already uses (`roomLightGroups`/`ensureRoomLightGroups`) — SPORTS reuses its own pre-existing dedicated group so its ambient rig and its curated picture lights share one toggle — so this is not ~10 more rooms' fixture rigs left always-on campus-wide. `MuseumBuilder.tsx`/`MuseumRoomPopup.tsx` (single-room preview scenes, no occupancy system) get the rig always-on via the same param's default (`lights` omitted → straight onto the scene) — neither file needed a source change.
- [x] Real campus geometry/doors/navigation/movement/collision/`CAMPUS_ROOMS`/`CAMPUS_DOORS` byte-for-byte unchanged — `git diff --stat` touches exactly 3 files (`campusRoomBuilder.ts`, `museumRoomFurniture.ts`, `VltdMuseumCampus.tsx`); `campusLayout.ts`/`museumStandard.ts` untouched. Gallery Builder's own files (`VirtualGalleryRoom.tsx`, `galleryRoomFinishes.ts`, `galleryTextures.ts`) read from, never modified.
- [x] `npx tsc --noEmit` clean (0 errors) / targeted ESLint on all 3 changed files, 0 errors (1 pre-existing, unrelated rule-suppression warning carried over) / `npm run build` clean, exit 0 — ran a real `npm install` first (this worktree had no installed dependencies).
- [ ] **Not live-verified.** No reliable way to judge from code alone whether this actually reads as good as the personal rooms — the parent session reviews this live in a real browser and iterates further if it doesn't hold up.

## 2026-09-12 (fixes pass) — Museum Builder: real camera movement, row-aligned wall items, Single/Dual/Three-row selector, Shelves checkbox, invented Background swatch removed for the real Wallpaper flow
EK's itemized feedback after her first live test of `/museum/builder` (the new page directly below). Full detail in the matching HANDOFF.md entry (same date, directly above the "new page" entry). Correction pass on `src/components/gallery/MuseumBuilder.tsx` only — same do-not-touch list as the original page.
- [x] Camera: scroll-to-step + WASD added, reusing `visitorController.ts`'s `buildKeyboardMoveDirection`/`easeTowardTargets`/`WHEEL_STEP`/`facingDirection` (the same functions `VltdMuseumCampus.tsx`/`VirtualGalleryRoom.tsx` already use) alongside the original pass's `applyDrag`/`aimCamera`. Collision is a simple rectangular clamp against the current room's own `roomBounds` (same technique as the accepted personal room's own fixed-box clamp).
- [x] Wall items now row-aligned: `computeRoomPlacementSlots` (`campusRoomBuilder.ts`) takes a new optional `rowCount` (1/2/3) that lays each wall span's own item count into a real column x row grid using the personal Gallery's own `SHELF_ROW_Y`/`shelfItemY()` (`galleryRoomSlots.ts`) reused directly for the row heights — no unit conversion needed (`MUSEUM_EYE_HEIGHT` === the personal room's own `eyeHeight`, both `3.6`). `maxHeight` per slot now scales with row count (1.3/1.9/2.2) so rows can't visually overlap. Left optional/undefined for every other existing caller (`VltdMuseumCampus.tsx`, `MuseumRoomPopup.tsx`) so neither is affected — byte-for-byte unchanged output for both.
- [x] New Single/Dual/Three-row segmented selector in the Room panel (Museum Builder only), independent of the existing Wall/Shelf/Case capacity sliders.
- [x] New Shelves checkbox — pure visual toggle on the existing wall-item slots, reusing `museumRoomFurniture.ts`'s `buildShelfBoard`/`createShelfMaterial` unchanged (drawn at each in-use row's real `SHELF_ROW_Y` height via new `wallRowBoardHeights()`). Not the same as the already-working Shelf-items/Case-items capacity sliders, left alone.
- [x] Invented "Background: Neutral/Warm Ivory/Cool Slate/Charcoal" swatch dropdown deleted entirely from `MuseumBuilder.tsx` (state, handler, and its `ROOM_BACKGROUND_OPTIONS`/`backgroundWallColorHex`/`setRoomBackground` imports all removed) — that preset system itself is untouched and stays a real, separate, already-accepted feature of `RoomEditorModal.tsx`. What remains is only the real wallpaper-image upload, relabeled "Wallpaper" to match the personal Gallery Builder's own actual button/flow (`handleWallpaperUpload`/`fileToRoomWallpaper`/`uploadHallWallpaper`) — same `uploadHallWallpaper()`/Storage bucket as before, just no more redundant preset UI next to it.
- [x] New migration `20260912_museum_room_wall_rows_and_shelves.sql` (`wall_row_count`/`wall_shelves_enabled` on `museum_room_meta`) — **not run**, SQL pasted in the session's final report for EK to run by hand. New `setRoomWallLayout()` in `museumCampusConfig.ts`; column-cascade fallback extended (FULL → WITH_CAPACITY → WITH_BACKGROUND → BASE) so this and the still-possibly-unrun prior capacity/background migration both keep failing soft independently.
- [x] `npx tsc --noEmit` clean (0 errors) / targeted ESLint 0 errors (1 pre-existing rule-suppression warning, not introduced by this pass) / `npm run build` clean.
- [ ] **Not live-verified** — no authenticated browser session available to this pass for the owner-gated `/museum/builder` route. Not declared accepted or ready for EK's test.

## 2026-09-12 (new page) — Museum Builder: separate owner-only page (`/museum/builder`), shelf/case furniture, per-room capacity sliders
EK, after using the in-Gallery-Builder museum room popup below: "duplicate this page but instead call it Museum Builder... Leave the original Gallery Builder alone though for now... no other user should have access to it." Full detail in the matching HANDOFF.md entry (same date, at the very top).
- [x] New route `/museum/builder` (`src/app/museum/builder/page.tsx`) → `MuseumBuilderOwnerGate.tsx` (role === "owner" specifically, patterned after `VltdMuseumAdminGate.tsx` but stricter — a granted non-owner admin is refused) → new `src/components/gallery/MuseumBuilder.tsx`, a structural duplicate of the Gallery Builder's page shell (identity card/Source/Room panel/toolbar) pointed at the real shared museum (`EDITABLE_ROOM_IDS`) instead of a personal Hall.
- [x] 3D view reuses MuseumRoomPopup.tsx's real-room-rendering approach (buildRoomShell/buildNeutralShell/buildSharedWall/buildRoomTrim, drag-to-look camera, only-this-room's wall segments) as a parallel copy, not an import — MuseumRoomPopup.tsx and `/museum/virtual-room` are both completely untouched. Organize reuses organizeSlots.tsx unchanged.
- [x] Shelf/case furniture ported (not redesigned) from VirtualGalleryRoom.tsx's own shelf-board/display-case recipe into new `src/lib/museumRoomFurniture.ts`; two new additive functions in `campusRoomBuilder.ts` (`computeRoomShelfSlots`/`computeRoomCaseSlots`) reuse the existing `computeUsableWallSpans`/`distributeAcrossSpans` math. Cases only offered where a room has a fully doorless wall and enough floor span — skipped elsewhere rather than forced.
- [x] Wall/Shelf/Case item-capacity sliders per room, debounced-persisted to 3 new `museum_room_meta` columns; slots recompute/redistribute live (pure functions of capacity).
- [x] Background control wired to the ALREADY-EXISTING `ROOM_BACKGROUND_OPTIONS`/`setRoomBackground` (built in the prior pass, never actually had an editor UI until now) plus a custom-image upload that reuses `virtualRooms.ts`'s `uploadHallWallpaper()` and its `room-wallpapers` Storage bucket directly — not a second, museum-only upload path — persisted to a new `background_image_url` column; applied in Museum Builder's own preview only (not wired into the live `VltdMuseumCampus.tsx`, a do-not-touch file for this pass).
- [x] New migration `20260912_museum_room_capacity_and_background.sql` (item_capacity/shelf_capacity/case_capacity/background_image_url on `museum_room_meta`) — **not run**, SQL pasted in the session's final report for EK to run by hand. Every new-column read fails soft.
- [x] `npx tsc --noEmit` clean (0 errors) / targeted ESLint 0 errors (2 pre-existing-style warnings, same rule-suppression notice already present in MuseumRoomPopup.tsx) / `npm run build` clean.
- [ ] **Not live-verified** — no authenticated browser session available to this pass for the owner-gated `/museum/builder` route; the parent session verifies live. Not declared accepted or ready for EK's test.

## 2026-09-12 (consolidation pass) — Shared Museum Room Editor: extracted the real Organize system instead of the prior passes' reinvented version
**Correction, not a new feature** — EK: "we spent 2 weeks creating a baseline in the 3D gallery, now you are trying to reinvent what we already have done." Full detail in the matching HANDOFF.md entry (same date, at the very top).
- [x] Extracted `VirtualGalleryRoom.tsx`'s real Organize system (state, `moveItemToSlot`/`focusSlot`/`confirmReplace`/`cancelReplace`/`removeFromSlot`, touch press-and-hold handlers, the numbered-overlay + Move-menu + Replace-confirm JSX) into new `src/components/gallery/organizeSlots.tsx` (`useSlotOrganizer()` hook + `<OrganizeSlotOverlay>`/`<OrganizeMoveMenu>`/`<OrganizeReplaceConfirm>`), generalized over an abstract slot list instead of the personal Gallery's own `selectedIds` array. `VirtualGalleryRoom.tsx`'s own rendered behavior/visuals unchanged.
- [x] `MuseumRoomItemPicker.tsx` **deleted** — `ItemPickerSheet.tsx` gained single-select mode (`mode`/`maxItems`/`pickerTitle` props), same tap-to-place UX, one real picker for both call sites.
- [x] `RoomEditorModal.tsx`: deleted "Enter Museum" link + the whole Background section (state, `saveBackground`, `ROOM_BACKGROUND_OPTIONS` import — dropped from scope; DB column left alone, untouched). "Add Items / Edit Room" now opens a new in-page popup instead of navigating away.
- [x] New `MuseumRoomPopup.tsx` — a focused view of just one room (its real shell/finish via the same `buildRoomShell`/`buildNeutralShell`/`buildSharedWall`/`buildRoomTrim` VltdMuseumCampus.tsx itself calls; only wall segments touching this room are built), camera fixed with drag-to-look only (no walking/collision), running the shared Organize overlay wired to `museum_room_items` via `slot_id`. `computeRoomPlacementSlots()` kept (real per-room wall geometry, still necessary).
- [x] `VltdMuseumCampus.tsx`: retired the old `?edit=<roomId>` full-page editor mode and its entire bespoke overlay/drag/pointer-event system (`SlotDragState`, `performMove`/`handleMoveTo`/`handlePickItem`/`handleRemoveSlot`, the rAF projection + window pointer-drag effects, the numbered-overlay/toolbar JSX) — all superseded by `MuseumRoomPopup.tsx`. `?room=` (plain spawn, no editing) untouched. Campus geometry/doors/lighting/camera/collision/Museum Map/personal Gallery Hall save flow untouched, per the work order.
- [x] `npx tsc --noEmit` clean (0 errors) / targeted ESLint 0 errors (12 warnings, all pre-existing or the same rule-suppression notice already present elsewhere) / `npm run build` clean, exit 0 — ran a real `npm ci` first (this worktree had no installed dependencies; lockfile byte-identical to the main checkout).
- [ ] **Not live-verified** — no authenticated browser session available to this pass for the admin-gated Map/Room Editor/`/museum/vltd`; the parent session verifies live. Not declared accepted or ready for EK's test.

## 2026-09-12 (follow-up pass) — Museum Room Editor: background application fix + drag interactions
Follow-up to the overnight pass directly below — EK asked for exactly the two items that pass's own gaps list flagged, nothing else touched. Full detail in the matching HANDOFF.md entry (same date, at the very top).
- [x] **Root cause fixed for background application**: `roomWallMaterial()` in `VltdMuseumCampus.tsx` used to cache one wall `Material` per `RoomFinish` OBJECT IDENTITY, so every room sharing a finish (SPORTS included) literally shared the same `Material` instance — recoloring one would have recolored them all, exactly what "SPORTS must never change COLLECTION/CARDS/HUB" forbids. Re-keyed the cache per ROOM ID instead — every room now owns its own Material/texture instance — then applied each room's saved `museum_room_meta.background_id` (new `backgroundWallColorHex()` in `campusRoomBuilder.ts`) to exactly that room's material once it loads. `buildSharedWall()`/`buildNeutralShell()`/`buildRoomShell()` untouched.
- [x] **Drag interactions added**, on top of (not replacing) the existing click-to-arm/click-destination flow and keyboard Move/Replace/Remove controls: real mouse press-drag-drop on desktop, long-press (450ms) then drag on mobile/tablet. Every input method — click-to-arm, mouse drag, long-press drag — calls the same extracted `performMove(sourceSlotId, targetSlotId)`, so autosave/occupied-destination-confirm/validity behave identically regardless of how the move started.
- [x] `npx tsc --noEmit` clean / targeted ESLint 0 errors (1 pre-existing, unrelated warning carried over) / `npm run build` clean, exit 0, all 138 routes including `/museum/vltd` — all three run for real against a fresh `npm ci` in this worktree (it had no installed dependencies at the start of this pass).
- [ ] **Migration `20260912_museum_room_placement.sql` still not applied** — reconfirmed missing (`slot_id`/`background_id` both absent) via a direct read-only query before this pass started; every read/write still fails soft exactly as before, so a saved background or slotted move genuinely cannot round-trip until EK runs it.
- [ ] **Not live-verified** — no authenticated Browser session available to this pass (same limitation as the overnight pass below); the parent session verifies live once the migration is confirmed run. This pass does not declare the feature accepted or ready for EK to test.

## 2026-09-12 (overnight pass) — Museum: real in-3D Room Editor with numbered placement slots, SPORTS + 8 other gallery rooms
Full detail in the matching HANDOFF.md entry (same date, near the top). Work order: `docs/MUSEUM-SHARED-ROOM-EDITOR-OVERNIGHT-PASS-2026-09-12.md`.
- [x] Shared placement engine: `computeRoomPlacementSlots()`/`placeItemsAtSlots()` (`campusRoomBuilder.ts`) + `deriveRoomDoorways()`/`EDITABLE_ROOM_IDS` (`campusLayout.ts`) — numbered, stable wall positions from real room geometry, the SAME generator for both the editor and the museum's own display.
- [x] Real in-3D editor: `?edit=<roomId>` on `/museum/vltd` spawns into that room and shows a numbered +/- overlay (real, tabbable buttons projected each frame, mirroring the personal Gallery's own Organize technique) — Add/Replace/Move (with replace-confirm)/Remove, autosave with Saving/Saved/Save-failed, Done returns via browser history.
- [x] Item picker (`MuseumRoomItemPicker.tsx`) sources the same `loadItems()` vault lookup already used elsewhere — no second item source.
- [x] SPORTS + all 8 other real gallery rooms (POP_CULTURE, TCG, MISC, BUILT_BOTANY, GAMES, AUTOMOTIVE, COLLECTION, CARDS) render through the same slot engine once curated — any room with zero curated items keeps its exact current behavior (vault placeholder, or SPORTS's "coming soon" plaque) unchanged.
- [x] `RoomEditorModal.tsx`: "Add Items / Edit Room" + "Enter Museum" links, item count/capacity, Background picker (4 swatches) for the 9 editable rooms; flat "+ Add item" form hidden only for those 9, untouched for every other room type.
- [x] Renamed room titles now also update the doorway destination sign and the in-museum top-of-screen label overlay (previously only the Map and the editor's own title field) — `CampusRoomId` itself untouched.
- [x] New migration `supabase/migrations/20260912_museum_room_placement.sql` (`slot_id` on `museum_room_items`, `background_id` on `museum_room_meta`) — **not yet run by EK, full SQL in the matching HANDOFF.md entry.** Every new-column read is a fails-soft double-select; nothing in this pass assumes it's been run.
- [x] `npx tsc --noEmit` / targeted ESLint (0 errors) / `npm run build` clean — both commits (`b42af71`, `f70e988`).
- [x] Both commits pushed to `main`; `b42af71`'s deployment confirmed **Ready** on Vercel production via the Vercel CLI (`vercel inspect`, authenticated as EK's own account, `dpl_8EPoDs4TeHhz2hnPLYpuuU2KNHw5`, aliased to `vltd.vercel.app`). `f70e988`'s deployment was still Building when this was written — see the Required Morning Report for its final status.
- [ ] **Not live-verified — a real blocker, not skipped quietly.** `/museum/vltd` is admin/owner-gated and the app requires login; this session's Claude-in-Chrome connection (this project's own standing convention for authed pages) was unreachable both times it was retried, and entering EK's password into the sandboxed Browser pane is against policy. The sandboxed pane only confirmed `GET /museum/vltd` returns `200` before its own login redirect — not the actual editor. **EK still needs to open the SPORTS edit badge herself** and review from the HUB door, COLLECTION door, CARDS door, and room center before this is accepted as the new standard, matching the work order's own required review.
- [ ] **Background choice is saved but not yet visually applied** in the live 3D scene — data layer + editor UI are real and ready; wiring a saved `background_id` into `VltdMuseumCampus.tsx`'s cached-by-identity wall materials is tracked as its own follow-up, not squeezed into this pass to avoid destabilizing the wall-building code the work order said to leave alone.
- [ ] "Move" is a click-to-arm/click-destination flow, not a literal continuous drag gesture (chosen for mouse/touch/keyboard reliability) — flag if EK specifically wants raw drag-and-drop instead of tap-to-place.

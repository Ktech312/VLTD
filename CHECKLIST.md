...
- [x] New Style control in `MuseumBuilder.tsx`: White/Vault/Arcade/Industrial Loft, matching `VirtualGalleryRoom.tsx`'s own style `<select>` labels/values exactly. Persisted via new `museum_room_meta.room_style` column (migration `20260912_museum_room_style.sql` — **✅ confirmed run by EK 2026-09-19**). Fails soft like every other optional column in `museumCampusConfig.ts`. Blue is NOT offered (no `createGalleryFinishes()` entry) — disclosed known gap.

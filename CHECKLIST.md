...
- [ ] **Museum artwork: 3 selectable frame styles, gated by shelf vs. no-shelf display.** EK's 3 screenshots (2026-09-14): (1) the current plain black frame with visible plastic sleeve, no mat — EK wants this style restricted to items displayed with NO shelves (flat wall-hang only, no shelf beneath). (2)/(3) a gold-trimmed white-mat frame — shown both in-museum on a shelf display and as the reference style from the actual Vault/Collection card UI (gold border, black mat, "COLLECTION PIECE" card look) — EK wants this style available specifically for wall-of-shelves displays. Overall ask: give the user 3 frame options to choose from (this is 2 of the 3 — third TBD/not yet specified), with the choice constrained by whether the display uses shelves. Needs to find wherever frame style/geometry is chosen relative to shelf-vs-no-shelf placement in the museum room/shelf-building code (`VltdMuseumCampus.tsx` / `hangFrame()` and whatever builds shelf displays) before scoping the actual work — this is a bigger feature (frame style selection UI + 2-3 new frame materials/geometries) than the other punch-list items, not a one-line fix.

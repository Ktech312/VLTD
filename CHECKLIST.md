...
## Next batch of fixes — EK's punch list, reported 2026-09-14, NOT started
All explicitly deferred by EK ("add that to the fix list") — logged here so they aren't lost, not investigated or fixed this session.
- [ ] **2FA modal: Enter key doesn't submit** — see the root cause already found in the entry directly above.
- [ ] **Museum (`/museum/vltd`): still getting stuck in doorways when scrolling in.** Not investigated this session. Likely area, based on this session's own recent work in the same file: `moveWithCollision()`'s substep march in `VltdMuseumCampus.tsx`, or the door-bridge width (`computeDoorBridges()`/`WALKABLE_MARGIN` in `campusLayout.ts`). Worth specifically checking whether the 2026-09-10 jamb/wall z-fighting fix (`4a0cb01`, which trimmed the solid wall piece back by the casing's own width at every door gap) or the new MISC↔HUB door (`1546288`) narrowed a walkable bridge or wall footprint enough to catch collision right at the jamb — both touched exactly this kind of geometry recently. Needs EK to say which doorway(s) specifically, and whether it's every door or a subset, before it can be root-caused.
- [ ] **Museum (`/museum/vltd`): very slow/choppy when looking up or turning sideways (drag-to-look).** Not investigated this session. Not obviously the same texture-payload issue already fixed (`a80b4f0`, ~17MB→~2.8MB) — that fix targeted load time, not runtime frame rate, so if this is ongoing per-frame stutter rather than a one-time load stall it's a different cause. Worth checking, in order: (1) real FPS/frame-timing during a drag (not just load timing) via `performance.now()` deltas in the render loop before assuming a cause; (2) whether this correlates with a specific room (e.g. HUB after the new Grand Hall GLB ceiling, `grand-hall-ceiling-blender-v1.glb` at 7.9MB and embedding its own textures — its own cold-cache/movement-smoothness measurement was flagged as not yet done in the 2026-09-13/14 Grand Hall entry below) versus being campus-wide. Needs EK to say whether it's one room or everywhere before it can be root-caused.
- [ ] **HUB Grand Hall skylight: remove the 4 free-floating ceiling lights down the center of the glass skylight.** EK's own annotated screenshot (2026-09-14) circles 4 lights hanging in open air along the skylight's centerline with nothing visibly supporting them — reads as floating props, not fixtures. Two acceptable fixes per EK: (a) remove them outright, or (b) keep them but add a visible chain/rod from the skylight structure down to each fixture so they read as hanging pendants. Likely in the Grand Hall ceiling/skylight build in `VltdMuseumCampus.tsx` (the 2026-09-13 custom design pass — "four warm point lights spread across the coffer ring" per that entry's own description above; confirm whether these are that same set or a separate set before touching anything, since the coffer ring lights are supposed to stay).
- [ ] **Museum artwork: frames need to sit fully in front of the wall, not straddling a wall-segment seam.** EK's annotated screenshot (2026-09-14, TCG room corner) circles two framed pieces with a dark vertical seam line running straight through the artwork itself — the wall-panel joint (or a wall-segment/corner boundary) is showing through/behind the frame instead of the frame sitting cleanly on one continuous wall face in front of it. Likely a `hangFrame()` placement or z-offset issue in `VltdMuseumCampus.tsx` — probably surfaced by the same per-segment wall-panel UV work from the 2026-09-10 pass (the seam is between two BoxGeometry wall segments), or the frame's forward z-offset from the wall isn't enough to clear the seam visually. Needs a check across other rooms/corners for the same issue before assuming TCG-only.
- [ ] **Museum artwork labels: restyle to match the door-sign plaque, sized for the artwork not the door.** EK's screenshot (2026-09-14, "Samurai: An Island at War" item card) shows the artwork's name label as a plain white rectangle with plain dark text — EK wants it to use the same sign/plaque style already built for the room-name signs mounted above each doorway (gold-framed dark plaque, e.g. the "TCG"/"POP CULTURE" signs seen in other screenshots this session), but scaled down to fit under/near the individual artwork instead of reusing the door sign's own (much larger) size. Likely wherever `hangFrame()` (or the artwork-label-building code near it) in `VltdMuseumCampus.tsx` builds the caption plane, versus wherever the door-sign plaque geometry/material is built — needs both located and the plaque style/material reused at a new, artwork-appropriate scale.

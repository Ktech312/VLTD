...
- [x] New migration `20260912_museum_room_wall_rows_and_shelves.sql` (`wall_row_count`/`wall_shelves_enabled` on `museum_room_meta`) — **✅ confirmed run by EK 2026-09-18.** New `setRoomWallLayout()` in `museumCampusConfig.ts`; column-cascade fallback (FULL → WITH_CAPACITY → WITH_BACKGROUND → BASE) is now live/exercised for real.
...
- [x] New migration `20260912_museum_room_capacity_and_background.sql` (item_capacity/shelf_capacity/case_capacity/background_image_url on `museum_room_meta`) — **✅ confirmed run by EK 2026-09-18.**
...
- [x] **Migration `20260912_museum_room_placement.sql` — ✅ confirmed run by EK 2026-09-18.** `slot_id`/`background_id` now real; a saved background or slotted move can round-trip.
...
- [x] New migration `supabase/migrations/20260912_museum_room_placement.sql` (`slot_id` on `museum_room_items`, `background_id` on `museum_room_meta`) — **✅ confirmed run by EK 2026-09-18.**
...
- [x] **New migration `20260912_museum_room_meta.sql` — ✅ confirmed run by EK 2026-09-18.** First attempt hit "policy already exists" (the table had already been created by an earlier, never-confirmed partial run, and this migration was missing the drop-if-exists guard every other migration uses) — fixed at the source and re-run clean.

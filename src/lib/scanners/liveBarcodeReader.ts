...
export function startLiveBarcodeScan(
  ...
  const { timeBetweenScansMillis = 400, onDiagnostic } = opts;
  const reader = new BrowserMultiFormatReader(buildHints(), timeBetweenScansMillis);
  // The not-found retry delay defaults to 0 in this lib, so the decode loop runs
  // flat-out and cooks the phone. Throttle it hard — a live scan doesn't need to
  // try more than a couple times a second.
  try {
    (reader as { timeBetweenDecodingAttempts?: number }).timeBetweenDecodingAttempts = 500;
  } catch {
    /* older lib without the setter — fine */
  }
  let stopped = false;
  let started = false;

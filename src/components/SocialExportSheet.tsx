...
// ─── Image proxy helper ───────────────────────────────────────────────────────
// Canvas drawImage rejects cross-origin images even with crossOrigin="anonymous"
// unless the server sends CORS headers. Route Supabase images through our proxy
// so the canvas gets a same-origin response it can freely draw.
...
function ImageCardTab({ item }: { item: VaultItem }) {
  ...
  // Auto-generate preview when tab opens so the user sees their card immediately
  ...
  useEffect(() => {
    ...
    void (async () => {
      setExporting(true);
      try {
        const dataUrl = await exportToCanvas({ item, ratio, bg, showValue, watermark, hashtags: Array.from(new Set(autoTags.slice(0, 8))), showHashtags: true });
        setPreview(dataUrl);
      } catch { /* ignore */ }
...
function CarouselTab({ currentItem }: { currentItem: VaultItem }) {
  ...
  async function handleExportAll() {
    ...
    const results: { dataUrl: string; slug: string }[] = [];
    for (let idx = 0; idx < items.length; idx++) {
      ...
      results.push({ dataUrl, slug });
    }

    // Try native share sheet first (iOS/Android) — lets the user post directly to Instagram, TikTok, etc.
    if (typeof navigator !== "undefined" && "share" in navigator) {
      try {
        const files = await Promise.all(
          results.map(async ({ dataUrl, slug }, idx) => {
            const res = await fetch(dataUrl);
            const blob = await res.blob();
            return new File([blob], `vltd-carousel-${idx + 1}-${slug}.png`, { type: "image/png" });
          })
        );
        if (navigator.canShare?.({ files })) {
          await navigator.share({ files, title: "My Collection · VLTD" });
          setExporting(false);
          setExported(true);
          return;
        }
        ...
        if ((e as Error).name === "AbortError") { setExporting(false); return; }
        // canShare returned false or share failed — fall through to downloads
      }
    }

    // Desktop / unsupported: staggered individual downloads
    for (let idx = 0; idx < results.length; idx++) {
      triggerDownload(results[idx].dataUrl, `vltd-carousel-${idx + 1}-${results[idx].slug}.png`);
  ...
  return (
    <div className="flex flex-col gap-5">
      <div className="rounded-2xl p-4 ring-1 ring-[color:var(--border)]" style={{ background: "var(--pill)" }}>
        <div className="text-xs font-semibold" style={{ color: "var(--muted)" }}>
          Select up to 10 items. Each gets a 1:1 square card. On iPhone/Android, tap Share to post directly to Instagram, TikTok, or any app. On desktop the files download to your device.
        </div>
      </div>
      ...
      <div className="flex gap-2">
        <button type="button" onClick={() => void handleExportAll()}
          ...
          {exporting ? `Generating ${selected.size} card${selected.size !== 1 ? "s" : ""}…` : exported ? "✓ Done!" : `Share ${selected.size} Card${selected.size !== 1 ? "s" : ""}`}
        </button>
      </div>

      <div className="text-[10px] text-center" style={{ color: "var(--muted)" }}>
        On desktop, files save one by one to your Downloads folder.
      </div>
    </div>
  );
}

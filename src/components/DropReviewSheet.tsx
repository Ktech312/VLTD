...
export default function DropReviewSheet({ session, onClose, onFinish }: Props) {
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="mt-0.5 truncate text-xs text-[color:var(--muted)]">
                    {[item.categoryLabel, item.grade].filter(Boolean).join(" · ") || "Tap to review item"}
                  </div>
                  {item.currentValue ? (
                ...
                <AppIcon name="next" strokeWidth={2} className="shrink-0 text-[color:var(--muted)]" />
              </button>
            ))
          )}
          {skipped.length > 0 ? (
            <div className="text-center text-xs text-[color:var(--muted)]">
              {skipped.length} removed from this drop.
            </div>
          ) : null}
        </div>

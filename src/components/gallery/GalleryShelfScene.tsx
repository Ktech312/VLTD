...
function getShelfThemeClasses(themePack?: string | null) {
  switch ((themePack || "classic").toLowerCase()) {
    case "walnut":
      return {
        wallOverlay: "bg-[linear-gradient(180deg,rgba(18,10,6,0.14),rgba(13,8,6,0.30))]",
        stage: "bg-[rgba(20,12,8,0.18)] ring-[#b98b62]/20 text-stone-100",
    ...
    case "midnight":
      return {
        wallOverlay: "bg-[linear-gradient(180deg,rgba(4,8,14,0.16),rgba(2,5,10,0.34))]",
        stage: "bg-[rgba(5,10,17,0.20)] ring-cyan-300/12 text-slate-100",
    ...
    case "marble":
      return {
        wallOverlay: "bg-[linear-gradient(180deg,rgba(255,255,255,0.08),rgba(52,58,68,0.16))]",
        stage: "bg-[rgba(255,255,255,0.14)] ring-slate-300/28 text-slate-900",
    ...
    default:
      return {
        wallOverlay: "bg-[linear-gradient(180deg,rgba(15,12,8,0.14),rgba(11,9,7,0.28))]",
        stage: "bg-[rgba(14,11,8,0.18)] ring-white/10 text-stone-100",
        ...
        {items.map((item) => (
          <div key={item.id} className="min-w-0">
            ...
            <Link href={`${galleryHrefPrefix}/${item.id}`} className="group block w-full">
              <div className={["relative w-full overflow-hidden rounded-[16px] ring-1 shadow-[0_10px_24px_rgba(0,0,0,0.18)]", theme.tile].join(" ")}>
...
export default function GalleryShelfScene({
  ...
  return (
    <section className="relative overflow-hidden rounded-[30px] ring-1 ring-white/10 shadow-[0_30px_90px_rgba(0,0,0,0.34)]">
      <div className="absolute inset-0 bg-[linear-gradient(180deg,rgba(8,10,14,0.30),rgba(8,10,14,0.44))]" />
      <div
        className="absolute inset-x-0 top-0 h-[46%] sm:h-[50%]"
        style={{
      ...
      />
      <div className={["absolute inset-x-0 top-0 h-[46%] sm:h-[50%]", theme.wallOverlay].join(" ")} />
      <div className="absolute inset-x-0 top-[42%] h-[18%] bg-[linear-gradient(180deg,rgba(8,10,14,0.0),rgba(8,10,14,0.48)_75%,rgba(8,10,14,0.72))]" />
      <div className="pointer-events-none absolute inset-x-0 top-0 h-40 bg-[radial-gradient(circle_at_top,rgba(255,255,255,0.10),transparent_42%)]" />
      ...
      <div className="relative px-4 py-5 sm:px-5 sm:py-6">
        ...
        <div className={["rounded-[26px] p-4 backdrop-blur-[1px] ring-1", theme.stage].join(" ")}>

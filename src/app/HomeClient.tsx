...
function GalleryCarousel({ galleries }: { galleries: Gallery[] }) {
  ...
  return (
    <section ref={sectionRef} style={{ touchAction: "pan-y", userSelect: "none" }}>
      <Link href={"/gallery/" + current.id} style={{ display: "block", width: "100%", height: "88px", overflow: "hidden", border: `1px solid ${C.bd}`, borderRadius: "7px", background: "rgba(10,18,35,0.9)" }}>
        {current.coverImage ? (
          // eslint-disable-next-line @next/next/no-img-element
              ...
              <div style={{ padding: "13px 15px" }}>
                {galleries.length > 0 ? (
                  ...
                  <div>
                    ...
                    <div style={{ fontFamily: C.r, fontSize: "15px", fontWeight: 600, color: C.text, marginTop: "10px" }}>No galleries yet</div>
                    <div style={{ fontSize: "11px", color: C.muted, marginTop: "3px" }}>Curate and share your collection.</div>
                    <Link href="/museum/new" style={{ display: "inline-flex", alignItems: "center", marginTop: "9px", fontSize: "11px", color: C.gold, textDecoration: "none" }}>Create Gallery →</Link>
                  </div>
                )}
              </div>
            </div>

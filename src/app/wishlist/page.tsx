...
/* ── Publisher pills for comic search ───────────────────── */
...
function ComicSearchCard({
  ...
  return (
    ...
    >
      ...
      <div style={{ padding: "8px 8px 10px", display: "flex", flexDirection: "column", gap: "3px", flex: 1 }}>
        <div style={{
          fontSize: "11px", fontWeight: 700, color: "#F0EAD6", lineHeight: 1.3,
        ...
        <div style={{ fontSize: "10px", color: "rgba(255,255,255,0.30)", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>
          {issue.publisher}
        </div>
        {issue.storeDate && (
    ...
    <div style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
          ...
          <div style={{ textAlign: "left" }}>
            ...
            <div style={{ fontSize: "12px", color: "rgba(240,234,214,0.4)", marginTop: "1px" }}>
              Search upcoming releases by artist or series — save to wishlist
            </div>
          </div>
        </div>
        <div style={{
      ...
      {expanded && (
        <div style={{ borderTop: "1px solid rgba(245,181,72,0.10)", padding: "14px 16px 16px" }}>
          {/* Mode toggle */}
          <div style={{ display: "flex", background: "rgba(255,255,255,0.05)", borderRadius: "10px", padding: "3px", marginBottom: "10px", width: "fit-content" }}>
            {(["creator", "series"] as const).map((m) => (
              <button key={m} onClick={() => { setSearchMode(m); setResult(null); setNotFound(false); }}
                style={{
          ...
          <form onSubmit={(e) => { e.preventDefault(); handleSearch(1); }}
            ...
            <input
              ...
              placeholder={searchMode === "creator" ? "Artist — e.g. Peach Momoko" : "Series title — e.g. Demon Days"}
              style={{
                flex: 1, background: "rgba(255,255,255,0.06)", border: "1px solid rgba(245,181,72,0.20)",
                borderRadius: "10px", padding: "10px 13px", fontSize: "14px", color: "#F0EAD6", outline: "none",
              }}
            />
          ...
          {result?.resolvedCreator?.name && (
            <div style={{
              background: "rgba(245,181,72,0.08)", border: "1px solid rgba(245,181,72,0.15)",
              borderRadius: "8px", padding: "8px 12px", marginBottom: "12px",
              fontSize: "12px", color: "rgba(245,181,72,0.85)",
            }}>
              Showing work by <strong style={{ color: "#F5B548" }}>{result.resolvedCreator.name}</strong>
              {" · "}{result.count} issue{result.count !== 1 ? "s" : ""}{allMode ? " (all time)" : ` in next ${days} days`}
            </div>
          )}

          {/* Not found */}
          {notFound && !loading && (
            <div style={{ textAlign: "center", padding: "24px 16px", color: "rgba(240,234,214,0.35)" }}>
              <div style={{ fontSize: "13px", fontWeight: 600 }}>Nothing found</div>
          ...
          {!loading && groups.map((group) => (
            <div key={group.date} style={{ marginBottom: "20px" }}>
              <div style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: "8px" }}>
                ...
                <div style={{ fontSize: "10px", color: "rgba(240,234,214,0.3)" }}>
                  {group.issues.length}
                </div>
              </div>
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(110px, 1fr))", gap: "8px" }}>
            ...
            <div style={{ display: "flex", justifyContent: "center", gap: "10px", marginTop: "8px" }}>
              {currentPage > 1 && (
                <button onClick={() => handleSearch(currentPage - 1)}
                  style={{ padding: "8px 16px", borderRadius: "8px", border: "1px solid rgba(245,181,72,0.2)", background: "transparent", color: "#F5B548", fontWeight: 600, cursor: "pointer", fontSize: "13px" }}>
                  Prev
                </button>
              )}
              {result.hasMore && (
                <button onClick={() => handleSearch(currentPage + 1)}
                  style={{ padding: "8px 16px", borderRadius: "8px", border: "1px solid rgba(245,181,72,0.2)", background: "rgba(245,181,72,0.08)", color: "#F5B548", fontWeight: 600, cursor: "pointer", fontSize: "13px" }}>
                  Load more
                </button>
              )}
            </div>
  ...
  return (
    <div style={{
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        {item.publisher && (
          <div style={{ fontSize: "11px", color: "var(--theme-text-muted, #A0956B)", marginTop: "2px" }}>
            {item.publisher}
          </div>
        )}
        {item.storeDate && (
...
export default function WishlistPage() {
  ...
  async function handleComicRemove(metronId: number) {
    ...
    setWishlistIds((prev) => {
      const next = new Set(prev);
      next.delete(metronId);
      return next;
    });
  }

  // General wishlist handlers
        ...
        {comicItems.length > 0 && (
          <div>
            <div style={{ display: "flex", alignItems: "center", gap: "10px", marginBottom: "12px" }}>
              <h2 className="text-base font-bold" style={{ color: "var(--theme-text-primary, #F0EAD6)" }}>
                Saved Comics
              </h2>
              <span style={{
          ...
          <div className="rounded-[24px] border p-8 text-center"
            ...
            <div className="mt-5 flex flex-wrap justify-center gap-3">
              <Link href="/vault/add" className="rounded-full px-5 py-2 text-sm font-semibold transition"
                style={{
                  background: "linear-gradient(135deg, var(--gold-dark, #8B6914) 0%, var(--gold-mid, #C8941F) 25%, var(--theme-gold, #F5B548) 50%, var(--gold-light, #FFE08A) 70%, var(--gold-mid, #C8941F) 100%)",

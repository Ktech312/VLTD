...
// demo seed compatible with your current vaultModel
        ...
        {rows.map((r) =>
          r.href ? (
            <Link
              key={r.label}
              href={r.href}
              className="flex items-center justify-between rounded-2xl bg-[color:var(--surface)] px-4 py-2 ring-1 ring-[color:var(--border)] hover:bg-[color:var(--input)] transition"
            >
...
export function AnalyticsDashboard(props: {
  ...
  universeKey?: string;        // UniverseKey-ish
  categoryLabel?: string;      // decoded label
  subcategoryLabel?: string;   // decoded label
}) {
  const scope = props.scope;
  const uKey = props.universeKey ? normUniverse(props.universeKey) : null;
  const cat = props.categoryLabel ? String(props.categoryLabel) : null;
  const sub = props.subcategoryLabel ? String(props.subcategoryLabel) : null;
  ...
  // Scope filtering (ALL -> Universe -> Category -> Subcategory)
  const scoped = useMemo(() => {
    ...
    if (cat) {
      pool = pool.filter((i: any) => String(i.categoryLabel ?? "").toLowerCase() === cat.toLowerCase());
    }

    if (sub) {
      pool = pool.filter((i: any) => String(i.subcategoryLabel ?? "").toLowerCase() === sub.toLowerCase());
    }
  ...
  const totals = useMemo(() => {
    const cost = scoped.reduce((s, i) => s + clamp(Number(i.purchasePrice ?? 0)), 0);
    const value = scoped.reduce((s, i) => s + clamp(Number(i.currentValue ?? 0)), 0);
    const g = value - cost;
  ...
  const breakdown = useMemo(() => {
    ...
    if (scope === "universe" && uKey) {
      const map = new Map<string, { label: string; count: number; value: number; gain: number }>();
      for (const i of scoped) {
        const label = String((i as any).categoryLabel ?? "—") || "—";
        const v = clamp(Number(i.currentValue ?? 0));
        const c = clamp(Number(i.purchasePrice ?? 0));
        const g = v - c;
        const row = map.get(label) ?? { label, count: 0, value: 0, gain: 0 };
        row.count += 1;
        row.value += v;
        row.gain += g;
        map.set(label, row);
      ...
      const rows = Array.from(map.values()).map((r) => ({
        ...r,
        key: r.label,
        href: r.label === "—" ? undefined : `/portfolio/universe/${uKey}/category/${encodeSeg(r.label)}`,
      }));
    ...
    if (scope === "category" && uKey && cat) {
      ...
      const rows = Array.from(map.values()).map((r) => ({
        ...
        href:
          r.label === "—"
            ? undefined
            : `/portfolio/universe/${uKey}/category/${encodeSeg(cat)}/subcategory/${encodeSeg(r.label)}`,
      }));
    ...
    // SUBCATEGORY has no further drill
    return [];
  }, [scope, scoped, rankMode, uKey, cat]);

  const allocBase = useMemo(() => {
    const totalValue = breakdown.reduce((s, r) => s + Math.max(0, r.value), 0);
  ...
  function exportCsv() {
    ...
    downloadTextFile(`vltd_${parts.join("_")}.csv`, csv, "text/csv");
  }
  ...
  const title =
    scope === "all"
      ? "Portfolio Analytics"
      : scope === "universe"
        ? `${UNIVERSE_LABEL[uKey ?? "MISC"]}`
        : scope === "category"
          ? `${UNIVERSE_LABEL[uKey ?? "MISC"]} • ${cat}`
          : `${UNIVERSE_LABEL[uKey ?? "MISC"]} • ${cat} • ${sub}`;
    ...
    <main className="min-h-screen bg-[color:var(--bg)] text-[color:var(--fg)]">
      <div className="mx-auto max-w-6xl px-5 py-10">
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-2 text-sm text-[color:var(--muted)]">
              Drill-down • $ vs % allocation • Top 5 • Export • Backend-ready structure
            </p>
          </div>
        ...
        <div className="mt-6 flex flex-col gap-3 rounded-3xl bg-[color:var(--surface)] p-5 ring-1 ring-[color:var(--border)] shadow-[var(--shadow-soft)]">
          ...
          <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
            <div className="text-xs text-[color:var(--muted2)]">Click rows to drill down. Search filters everything.</div>
            ...
            <div className="flex items-center gap-2">
              ...
              <PillSelect<ViewMode>
                ...
                options={[
                  { value: "bars", label: "Graphs", subtitle: "Allocation bars" },
                  { value: "donut", label: "Donut", subtitle: "Allocation distribution" },
                  { value: "sparklines", label: "Sparklines", subtitle: "Quick trend hint" },
                ]}
                align="right"
                minWidthPx={0}
                extraWidthPx={10}
        ...
        <div className="mt-6 grid gap-3 md:grid-cols-3">
          <Card title="TOTAL" right={<div className="text-xs text-[color:var(--muted2)]">ROI {fmtPct(totals.roi)}</div>}>
            ...
            <div className="mt-1 text-sm text-[color:var(--muted)]">
              Cost {fmtMoney(totals.cost)} • Gain {fmtMoney(totals.gain)} • Items {scoped.length}
            </div>
          </Card>
                ...
                >
                  <div className="min-w-0">
                    ...
                    <div className="text-xs text-[color:var(--muted)] truncate">
                      {r.categoryLabel}
                      {r.subcategoryLabel !== "—" ? ` • ${r.subcategoryLabel}` : ""} • Gain {fmtMoney(r.gain)}
                    </div>
                  </div>
        ...
        <div className="mt-6 rounded-3xl bg-[color:var(--surface)] p-6 ring-1 ring-[color:var(--border)] shadow-[var(--shadow-soft)]">
          <div className="flex items-start justify-between gap-3">
            <div>
              ...
              <div className="mt-1 text-sm text-[color:var(--muted)]">Real daily snapshots (local). Range: {rangeLabel}.</div>
            </div>
            <div className="text-sm font-semibold text-[color:var(--fg)]">{fmtMoney(totals.value)}</div>
          </div>

          <div className="mt-4 rounded-2xl bg-[color:var(--input)] p-4 ring-1 ring-[color:var(--border)]">
            {historySeries.values.length >= 2 ? (
              <MiniLineChart values={historySeries.values} height={80} />
            ) : (
              <div className="text-sm text-[color:var(--muted)]">History appears after at least 2 days of snapshots.</div>
            )}
          </div>
        </div>
        ...
        <div className="mt-6 rounded-3xl bg-[color:var(--surface)] p-6 ring-1 ring-[color:var(--border)] shadow-[var(--shadow-soft)]">
          ...
          <div className="mt-2 text-xl font-semibold">{breakdownTitle}</div>
          <div className="mt-1 text-sm text-[color:var(--muted)]">
            Metric: {rankMode === "value" ? "Value" : "Gain"} • Allocation: {allocMode === "dollars" ? "$" : "%"} (based on value)
          </div>

          <div className="mt-5 rounded-3xl bg-[color:var(--input)] p-5 ring-1 ring-[color:var(--border)]">
            {view === "donut" ? (
              <Donut
                rows={breakdown.map((r) => ({ label: r.label, value: r.value, href: r.href }))}
                total={breakdown.reduce((s, r) => s + Math.max(0, r.value), 0)}
              />
            ) : view === "sparklines" ? (
              <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                {breakdown.map((r) => {
                  const allocPct = (Math.max(0, r.value) / allocBase) * 100;
                  ...
                  return (
                    <Link
                      ...
                      className={[
                        ...
                        r.href ? "hover:bg-[color:var(--input)]" : "opacity-70 pointer-events-none",
                      ].join(" ")}
                    >
                      ...
                      <div className="mt-3">
                        <MiniSparkline values={[0, Math.max(0, rankMode === "value" ? r.value : r.gain)]} />
                      </div>
                    </Link>
                  );
                })}
              ...
              <div className="grid gap-2">
                {breakdown.map((r) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="mt-3 h-2 overflow-hidden rounded-full bg-[color:var(--input)] ring-1 ring-[color:var(--border)]">
                        <div
                          className="h-full"
                          style={{
                ...
                {breakdown.length === 0 ? <div className="text-sm text-[color:var(--muted)]">No data.</div> : null}
              </div>
            )}
          </div>
        </div>
        ...
        <div className="mt-6 rounded-3xl bg-[color:var(--surface)] p-6 ring-1 ring-[color:var(--border)] shadow-[var(--shadow-soft)]">
          ...
          <div className="mt-4 overflow-hidden rounded-2xl ring-1 ring-[color:var(--border)]">
            <div className="grid grid-cols-12 gap-2 bg-[color:var(--input)] px-4 py-3 text-xs text-[color:var(--muted)]">
            ...
            <div className="divide-y divide-[color:var(--border)] bg-[color:var(--surface)]">
              {tableRows.map((r) => (
                <Link
                  key={r.id}
                  href={`/vault/item/${r.id}`}
                  className="grid grid-cols-12 gap-2 px-4 py-3 hover:bg-[color:var(--input)] transition"
                >
                  <div className="col-span-5 min-w-0">
                    <div className="text-sm font-semibold truncate">{r.title}</div>
                    <div className="mt-0.5 text-xs text-[color:var(--muted)]">
                      {r.categoryLabel}
                      {r.subcategoryLabel !== "—" ? ` • ${r.subcategoryLabel}` : ""} • Added {fmtMonthDay(r.added)} • Gain{" "}
                      {fmtMoney(r.gain)}
                    </div>
                  </div>
